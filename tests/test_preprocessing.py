...
from src.preprocessing.dem.hydrological_conditioning import (
    HydrologicalConditioner,
...
class TestDEMPreprocessing:
    ...
    def sample_dem_data(self):
        ...
        dem[hill_mask] += 30 * np.exp(
            -((X[hill_mask] - 7) ** 2 + (Y[hill_mask] - 7) ** 2)
        )
    ...
    def test_sink_filling(self, hydrological_conditioner):
        ...
        assert np.all(
            filled_dem >= original_dem
        ), "Filled DEM should have higher or equal elevations"
        assert not np.array_equal(
            filled_dem, original_dem
        ), "Sink filling should modify the DEM"
    ...
    def test_flow_direction_calculation(self, hydrological_conditioner):
        ...
        assert (
            len(unique_values) <= 9
        ), "D8 flow direction should have at most 9 unique values"
    ...
    def test_flow_accumulation_calculation(self, hydrological_conditioner):
        ...
        assert np.max(flow_acc) > np.min(
            flow_acc
        ), "Flow accumulation should vary across the DEM"
    ...
    def test_complete_conditioning_workflow(self, hydrological_conditioner):
        ...
        assert set(unique_stream_values).issubset(
            {0, 1}
        ), "Streams should be binary (0 or 1)"

    def test_dem_quality_validation(self, hydrological_conditioner):
        """Test DEM quality validation metrics."""
        results = hydrological_conditioner.condition_dem()
        metrics = validate_dem_quality(
            results["conditioned_dem"], results["flow_accumulation"]
        )
...
class TestTerrainFeatureExtraction:
    ...
    def test_slope_calculation(self, feature_extractor):
        ...
        np.testing.assert_allclose(
            slope_deg[valid_mask], slope_rad[valid_mask] * 180 / np.pi, rtol=1e-5
        )
    ...
    def test_curvature_calculation(self, feature_extractor):
        ...
        for curv_type in curvature_types:
            ...
            assert (
                len(valid_curv) > 0
            ), f"{curv_type} curvature should have valid values"
    ...
    def test_extract_all_features(self, feature_extractor):
        ...
        for name, feature_array in features.items():
            assert (
                feature_array.shape == feature_extractor.dem.shape
...
class TestHANDCalculation:
    ...
    def test_find_nearest_stream(self, hand_calculator):
        ...
        if nearest_row is not None and nearest_col is not None:
            ...
            distance = np.sqrt(
                (nearest_row - test_row) ** 2 + (nearest_col - test_col) ** 2
            )
            assert distance <= 20
...
class TestIntegrationWorkflows:
    ...
    def complex_terrain_file(self, tmp_path):
        ...
        transform = from_bounds(
            west=0, south=0, east=2000, north=2000, width=width, height=height
        )
    ...
    def test_performance_with_large_data(self, tmp_path):
        ...
        transform = from_bounds(
            west=0, south=0, east=5000, north=5000, width=width, height=height
        )
