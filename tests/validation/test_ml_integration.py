...
class TestMLDataValidator:
    ...
    def test_memory_intensive_dataset(self):
        ...
        if (
            "memory_analysis" in result.details
            and "estimated_memory_gb" in result.details["memory_analysis"]
        ):
...
class TestModelPerformanceValidator:
    ...
    def create_training_history(self, converged=True, overfitting=False, epochs=20):
        ...
        if converged:
            ...
            train_loss = [
                1.0 - (i * 0.8) / epochs + np.random.normal(0, 0.01)
            ...
            train_loss = [
                0.5 + 0.3 * np.sin(i * 0.5) + np.random.normal(0, 0.05)
                for i in range(epochs)
            ]
        ...
        if overfitting:
            ...
            val_loss = train_loss[:10] + [
                train_loss[9] + (i - 9) * 0.05 for i in range(10, epochs)
            ]
        else:
    ...
    def test_good_model_performance(self):
        ...
        model_results = {
            "training_history": self.create_training_history(
                converged=True, overfitting=False
            ),
            "test_metrics": {
                "accuracy": 0.85,
                "precision": 0.82,
    ...
    def test_non_convergent_model(self):
        ...
        model_results = {
            "training_history": self.create_training_history(
                converged=False, overfitting=False
            ),
    ...
    def test_overfitting_model(self):
        ...
        model_results = {
            "training_history": self.create_training_history(
                converged=True, overfitting=True
            ),
    ...
    def test_slow_inference(self):
        ...
        assert any(
            "slow" in issue.lower() or "speed" in issue.lower()
    ...
    def test_real_vs_dummy_comparison(self):
        ...
        model_results = {
            ...
            "real_data_metrics": {"accuracy": 0.82, "precision": 0.80, "recall": 0.84},
            "dummy_data_metrics": {
    ...
    def test_dummy_better_than_real(self):
        ...
        model_results = {
            "test_metrics": {"accuracy": 0.75},
            "real_data_metrics": {"accuracy": 0.70},  # Real data worse
            "dummy_data_metrics": {"accuracy": 0.78},  # Dummy better (concerning)
        }
...
class TestMLIntegrationValidator:
    ...
    def test_partial_ml_validation(self):
        ...
        ml_data = {"dataset": MockFloodDataset(size=50)}
    ...
    def test_ml_report_generation(self):
        ...
        assert (
            report["ml_validation_summary"]["overall_ml_score"] == 0.825
        )  # Average of 0.9 and 0.75
    ...
    def test_failed_ml_pipeline(self):
        ...
        labels = np.random.choice(
            [0, 1], size=(50, 256, 256), p=[0.999, 0.001]
        )  # Severe imbalance

        ml_data = {
            "dataset": dataset,
            "labels_data": {"labels": labels},
            "model_results": {"test_metrics": {"accuracy": 0.5}},  # Poor performance
        }
    ...
    def test_realistic_ml_workflow(self):
        ...
        class RealisticFloodDataset(Dataset):
            ...
            def __getitem__(self, idx):
                ...
                rainfall_channel = (
                    torch.exponential(torch.ones(1, 256, 256) * 0.1) * 50
                )  # Rainfall
                landuse_channel = torch.randint(
                    0, 5, (1, 256, 256)
                ).float()  # Land use categories

                input_tensor = torch.cat(
                    [dem_channel, rainfall_channel, landuse_channel], dim=0
                )
        ...
        ml_data = {
            ...
            "model_results": {
                ...
                "real_data_metrics": {"accuracy": 0.86, "iou": 0.76},
                "dummy_data_metrics": {
