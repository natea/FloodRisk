...
class TestRainfallValidator:
    ...
    def setup_method(self):
        ...
        self.validator = RainfallValidator(
            {"max_intensity": 200, "min_coverage": 0.9, "missing_data_threshold": 0.15}
        )

    def create_test_rainfall_array(
        self,
        shape=(50, 50),
    ...
    def test_rainfall_extreme_values(self):
        ...
        assert result.score < 1.0
        assert any(
            "extreme" in issue.lower() or "intensity" in issue.lower()
...
class TestSpatialConsistencyValidator:
    ...
    def setup_method(self):
        ...
        self.validator = SpatialConsistencyValidator(
            {"spatial_tolerance": 0.1, "min_overlap": 0.9}
        )
        self.temp_dir = Path(tempfile.mkdtemp())
    ...
    def test_non_overlapping_extents(self):
        ...
        raster2 = self.create_test_raster(
            "raster2.tif", bounds=(-100, 50, -90, 60)
        )  # No overlap
    ...
    def test_different_resolutions(self):
        ...
        raster1 = self.create_test_raster(
            "raster1.tif", shape=(100, 100)
        )  # Fine resolution
        raster2 = self.create_test_raster(
            "raster2.tif", shape=(50, 50)
        )  # Coarse resolution
...
class TestSimulationValidator:
    ...
    def create_test_simulation_results(
        self,
        add_negatives=False,
        extreme_depths=False,
        add_convergence=True,
        add_mass_balance=True,
    ):
        ...
        results = {"depths": depths, "velocities": velocities}
    ...
    def test_mass_conservation_issues(self):
        ...
        assert validation_result.score < 1.0
        assert any(
            "mass" in issue.lower() or "conservation" in issue.lower()
...
class TestTileQualityValidator:
    ...
    def setup_method(self):
        ...
        self.validator = TileQualityValidator(
            {"target_flood_ratio": (0.1, 0.9), "min_tiles": 50, "edge_threshold": 3}
        )
    ...
    def create_test_tiles(
        ...
        for i in range(num_tiles):
            ...
            tile_data = (tile_data > threshold).astype(float) * np.random.uniform(
                0.1, 2.0, size
            )
...
class TestPipelineValidator:
    ...
    def test_partial_pipeline_validation(self):
        ...
        pipeline_data = {"rainfall_data": np.random.uniform(0, 50, (50, 50))}
