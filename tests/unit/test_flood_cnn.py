...
class TestFloodCNN:
    ...
    def test_model_training_step(self, sample_flood_cnn_input, sample_flood_target, pytorch_device):
    ...
    @pytest.mark.slow
    def test_model_gradient_flow(self, sample_flood_cnn_input, sample_flood_target, pytorch_device):
    ...
    def test_model_with_different_input_sizes(self, pytorch_device):
        ...
        for batch_size in [1, 2, 4]:
            inputs = {
                "elevation_256m": torch.rand(batch_size, 1, 256, 256).to(pytorch_device),
                "elevation_512m": torch.rand(batch_size, 1, 128, 128).to(pytorch_device),
                "elevation_1024m": torch.rand(batch_size, 1, 64, 64).to(pytorch_device),
                "terrain_features": torch.rand(batch_size, 4, 256, 256).to(pytorch_device),
