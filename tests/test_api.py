    ...
    def predict(
        ...
        **kwargs,
        ...
        flood_depth = np.maximum(
            0, np.random.exponential(rainfall_intensity / 10, (height, width))
        )
...
class MockValidator:
    ...
    def validate_predictions(
        self, predictions: np.ndarray, ground_truth: np.ndarray, metrics_requested: list
    ) -> Dict[str, float]:
...
async def predict_flood(request: PredictionRequest, dem_file: UploadFile = File(...)):
    ...
    try:
        ...
        if dem_file.content_type not in ["image/tiff", "application/octet-stream"]:
            raise HTTPException(
                status_code=400, detail="Invalid file type. Expected GeoTIFF."
            )
        ...
        if request.rainfall_intensity < 0 or request.rainfall_intensity > 200:
            raise HTTPException(
                status_code=400, detail="Rainfall intensity must be between 0-200 mm/h"
            )

        if request.rainfall_duration < 0 or request.rainfall_duration > 1440:
            raise HTTPException(
                status_code=400,
                detail="Rainfall duration must be between 0-1440 minutes",
            )
...
async def validate_predictions(request: ValidationRequest):
    ...
    try:
        ...
        if len(predictions) != len(ground_truth):
            raise HTTPException(
                status_code=400,
                detail="Prediction and ground truth data must have same size",
            )
        ...
        if size * size != len(predictions):
            raise HTTPException(
                status_code=400, detail="Data must represent square arrays"
            )
        ...
        "models": [
            {
                ...
                "max_extent_km2": 1000,
                "accuracy_metrics": {"mae": 0.23, "rmse": 0.41, "r2": 0.85},
            },
            {
                "name": "flood_cnn_v2",
                "version": "2.0.0",
                ...
                "input_resolution": "5m",
                "max_extent_km2": 500,
                "accuracy_metrics": {"mae": 0.18, "rmse": 0.35, "r2": 0.89},
            },
        ]
    }
...
class TestAPIEndpoints:
    ...
    def test_validation_endpoint(self):
        ...
        ground_truth = predictions + np.random.normal(0, 0.1, (size, size)).astype(
            np.float32
        )
...
class TestAPIErrorHandling:
    ...
    def test_invalid_file_type_error(self):
        ...
        prediction_request = {"rainfall_intensity": 25.0, "rainfall_duration": 120}
    ...
    def test_invalid_rainfall_parameters(self):
        ...
        prediction_request = {"rainfall_intensity": -5.0, "rainfall_duration": 120}
    ...
    def test_missing_required_fields(self):
        ...
        response = client.post("/validate", json={})  # Missing required fields
...
class TestAPIPerformance:
    ...
    def test_concurrent_requests(self):
        ...
        ), "Concurrent execution not faster than sequential"
    ...
    def test_large_file_upload_performance(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".tif", delete=False) as tmp_file:
            ...
            prediction_request = {"rainfall_intensity": 30.0, "rainfall_duration": 180}
            ...
            ), f"Processing too slow: {processing_time}s for {file_size_mb:.1f}MB"
    ...
    def test_memory_usage_during_processing(self):
        ...
        for i in range(5):
            ...
            assert (
                memory_increase < max_increase_mb * 1024 * 1024
...
class TestAPIProduction:
    ...
    def test_input_sanitization(self):
        ...
        for malicious_input in malicious_inputs:
            ...
            ), f"Unsanitized input found in response: {malicious_input}"
