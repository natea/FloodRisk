...
from src.models.flood_cnn import (
    FloodDepthPredictor, PhysicsInformedLoss, DimensionlessFeatureProcessor,
    MultiScaleInputProcessor, RainfallScalingModule, FloodModelTrainer,
    create_flood_model, AttentionFusion, EncoderBlock, DecoderBlock
)
...
class TestFloodCNNArchitecture:
    ...
    def sample_input_data(self):
        ...
        return {
            ...
            'elevation': torch.randn(batch_size, 1, height, width) * 100 + 500,  # Realistic elevation
            'slope': torch.rand(batch_size, 1, height, width) * 0.5,  # 0-0.5 slope ratio
    ...
    def rainfall_stats(self):
        ...
        batch_size = 2
        return torch.tensor([
            [25.5, 120.0, 0.3],  # intensity (mm/h), duration (min), antecedent moisture
            [15.2, 180.0, 0.7]
        ], dtype=torch.float32)
    ...
    def test_model_forward_pass(self, flood_model, sample_input_data, rainfall_stats):
        ...
        if 'uncertainty' in predictions:
            ...
            assert torch.all(uncertainty_pred >= 0), "Uncertainty should be non-negative"
    ...
    def test_rainfall_scaling_module(self, sample_input_data, rainfall_stats):
        ...
        assert not torch.allclose(scaled_rainfall, rainfall_map), "Scaling should modify rainfall"
    ...
    def test_physics_informed_loss(self, sample_input_data):
        ...
        predictions = {
            'depth': torch.rand(batch_size, 1, height, width) * 5
        }
    ...
    def test_model_gradients(self, flood_model, sample_input_data, rainfall_stats):
        ...
        for name, param in flood_model.named_parameters():
            if param.grad is not None:
                has_gradients = True
                assert torch.all(torch.isfinite(param.grad)), f"Infinite gradients in {name}"
    ...
    def test_model_memory_efficiency(self, flood_model, sample_input_data, rainfall_stats):
...
class TestModelTraining:
    ...
    def test_validation_step(self, trainer_setup, training_batch):
        ...
        expected_keys = {'total_loss', 'mse_loss', 'mass_conservation_loss', 'mse', 'mae'}
    ...
    def test_training_loop_simulation(self, trainer_setup, training_batch):
        ...
        final_loss = losses[-1]
        assert final_loss <= initial_loss * 2, "Loss should not increase dramatically during training"
    ...
    def test_model_state_dict_save_load(self, trainer_setup):
        ...
        with tempfile.NamedTemporaryFile(suffix='.pth', delete=False) as tmp_file:
            ...
            for (name1, param1), (name2, param2) in zip(
                ...
                assert name1 == name2
                assert torch.allclose(param1, param2), f"Parameters {name1} don't match after save/load"
    ...
    def test_gradient_clipping(self, trainer_setup, training_batch):
        ...
        trainer, device = trainer_setup
        optimizer = torch.optim.Adam(trainer.model.parameters(), lr=0.01)  # Higher LR for larger gradients
        ...
        max_grad_norm = 1.0
        grad_norm_after = torch.nn.utils.clip_grad_norm_(trainer.model.parameters(), max_grad_norm)
        
        if grad_norm_before > max_grad_norm:
            assert grad_norm_after <= max_grad_norm * 1.01, "Gradients should be clipped"
...
class TestModelProductionReadiness:
        ...
        for case_idx, inputs in enumerate(extreme_cases):
            with torch.no_grad():
                try:
                    ...
                    assert torch.all(torch.isfinite(predictions['depth'])), \
                        f"Non-finite predictions in case {case_idx}"
                    
                    # Predictions should be reasonable (not extreme)
                    assert torch.max(predictions['depth']) < 1e3, \
    ...
    def test_model_serialization(self, production_model):
        ...
        test_input = {
            'high_res': torch.randn(1, 6, 64, 64)
        }
