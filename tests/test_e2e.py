...
from src.models.flood_cnn import FloodDepthPredictor, create_flood_model, FloodModelTrainer
...
class FloodRiskPipeline:
    ...
    def preprocess_data(self, dem_path: str, output_dir: str) -> dict:
        ...
        feature_stack = self._create_feature_stack(terrain_features, conditioning_results)
    ...
    def prepare_training_data(self, features: dict, synthetic_targets: bool = True) -> dict:
        ...
        training_data = {
            ...
            'data_statistics': self._calculate_data_statistics(feature_tensor, target_tensor)
        }
        
        logger.info(f"Training data prepared: {train_data[0].shape[0]} train, {val_data[0].shape[0]} val samples")
    ...
    def evaluate_model(self, test_data: dict = None) -> dict:
        ...
        if test_data is None:
            ...
            if test_features is not None:
                ...
                test_targets = torch.from_numpy(test_targets).float().unsqueeze(0).unsqueeze(0)
            else:
        ...
        if 'uncertainty' in predictions:
            ...
            uncertainty_metrics = self._analyze_uncertainty(pred_np, target_np, uncertainty_np)
        ...
        logger.info(f"Performance Summary: {evaluation_results['model_performance_summary']}")
    ...
    def run_full_pipeline(self, dem_path: str, output_dir: str) -> dict:
        ...
        try:
            ...
            # Step 3: Train model
            training_results = self.train_model(training_data, num_epochs=5)  # Reduced for testing
            ...
            logger.info(f"Full E2E pipeline completed successfully in {pipeline_time:.2f} seconds")
            return full_results
    ...
    # Helper methods
    def _create_feature_stack(self, terrain_features: dict, conditioning_results: dict) -> np.ndarray:
        ...
        for feature in features_to_stack:
            ...
            if np.max(feature_clean) > np.min(feature_clean):
                feature_norm = (feature_clean - np.min(feature_clean)) / (np.max(feature_clean) - np.min(feature_clean))
            else:
        ...
        feature_stack = np.stack(normalized_features, axis=0)  # Shape: (channels, height, width)
        
        return feature_stack.astype(np.float32)
    
    def _generate_synthetic_flood_targets(self, terrain_features: dict, shape: tuple) -> np.ndarray:
        ...
        twi_norm = np.clip((twi_clean - np.min(twi_clean)) / (np.max(twi_clean) - np.min(twi_clean)), 0, 1)
        ...
        for _ in range(num_flood_centers):
            ...
            radius = np.random.randint(min(height, width) // 10, min(height, width) // 5)
    ...
    def _spatial_train_val_split(self, features: torch.Tensor, targets: torch.Tensor, val_fraction: float = 0.2):
    ...
    def _get_feature_names(self) -> list:
        ...
        return [
            'elevation', 'slope', 'curvature', 'flow_accumulation', 
            'wetness_index', 'hand'
        ]
    
    def _calculate_data_statistics(self, features: torch.Tensor, targets: torch.Tensor) -> dict:
        ...
        return {
            'feature_stats': {
                ...
                'min': torch.min(features.view(features.shape[0], features.shape[1], -1), dim=2)[0].min(0)[0].tolist(),
                'max': torch.max(features.view(features.shape[0], features.shape[1], -1), dim=2)[0].max(0)[0].tolist()
            },
            'target_stats': {
                ...
                'flood_fraction': (torch.sum(targets > 0.1) / torch.numel(targets)).item()
            }
        }
    
    def _calculate_comprehensive_metrics(self, predictions: np.ndarray, targets: np.ndarray) -> dict:
        ...
        if len(pred_valid) == 0:
            return {key: float('nan') for key in ['mse', 'rmse', 'mae', 'r2', 'flood_accuracy']}
        
        # Basic regression metrics
        mse = mean_squared_error(target_valid, pred_valid)
        rmse = np.sqrt(mse)
        mae = mean_absolute_error(target_valid, pred_valid)
        r2 = r2_score(target_valid, pred_valid) if len(np.unique(target_valid)) > 1 else 0.0
        ...
        correlation = np.corrcoef(pred_valid, target_valid)[0, 1] if len(np.unique(pred_valid)) > 1 else 0.0
    ...
    def _analyze_spatial_patterns(self, predictions: np.ndarray, targets: np.ndarray) -> dict:
        ...
        return {
            'spatial_correlation': float(spatial_corr) if not np.isnan(spatial_corr) else 0.0
        }
    
    def _analyze_uncertainty(self, predictions: np.ndarray, targets: np.ndarray, uncertainty: np.ndarray) -> dict:
        ...
        return {
            'uncertainty_error_correlation': float(error_unc_corr) if not np.isnan(error_unc_corr) else 0.0,
    ...
    def _save_preprocessing_results(self, conditioning: dict, features: dict, feature_stack: np.ndarray, output_dir: str):
...
class TestE2EFloodRiskPipeline:
    ...
    def test_full_pipeline_execution(self, pipeline_config, sample_dem_file, tmp_path):
        ...
        assert results['pipeline_metadata']['success'], f"Pipeline failed: {results['pipeline_metadata'].get('error', 'Unknown error')}"
        ...
        assert final_loss <= initial_loss * 2, "Training loss should not increase dramatically"
    ...
    def test_preprocessing_pipeline(self, pipeline_config, sample_dem_file, tmp_path):
        ...
        required_terrain_keys = {
            'slope_degrees', 'curvature_total', 'flow_accumulation',
            'wetness_index', 'hand'
        }
    ...
    def test_training_pipeline(self, pipeline_config):
        ...
        pipeline.preprocessing_results = {'conditioning': {'conditioned_dem': np.random.randn(height, width)}, **mock_features}
        ...
        # Check tensor shapes
        assert training_data['train_features'].dim() == 4  # (batch, channels, height, width)
    ...
    def test_model_evaluation(self, pipeline_config):
        ...
        for metric_name, value in metrics.items():
            ...
            if not np.isnan(value):
                assert np.isfinite(value), f"Metric {metric_name} is not finite: {value}"
    ...
    def test_pipeline_performance_requirements(self, pipeline_config, sample_dem_file, tmp_path):
        ...
        assert abs(reported_time - total_time) < 5, "Reported time doesn't match measured time"
    ...
    def test_pipeline_configuration_validation(self, sample_dem_file, tmp_path):
        ...
        configurations = [
            {'name': 'minimal', 'config': {'stream_threshold': 100, 'base_channels': 16, 'depth_levels': 2}},
            {'name': 'standard', 'config': {'stream_threshold': 500, 'base_channels': 32, 'depth_levels': 3}},
            {'name': 'complex', 'config': {'stream_threshold': 1000, 'base_channels': 64, 'depth_levels': 4}}
        ]
...
class TestProductionReadiness:
    ...
    def test_system_integration_validation(self, tmp_path):
        ...
        assert processing_time < 300, f"Production pipeline too slow: {processing_time:.2f}s"
        
        # Accuracy requirements
        metrics = results['evaluation_results']['metrics']
        assert metrics['rmse'] < 1.0, f"RMSE too high for production: {metrics['rmse']:.3f}"
        assert metrics['mae'] < 0.6, f"MAE too high for production: {metrics['mae']:.3f}"
        assert metrics['r2'] > 0.6, f"R² too low for production: {metrics['r2']:.3f}"
        assert metrics['flood_accuracy'] > 0.75, f"Flood accuracy too low: {metrics['flood_accuracy']:.3f}"
    ...
    def test_production_resource_monitoring(self, tmp_path):
        ...
        assert memory_increase_mb < 500, f"Memory increase too high: {memory_increase_mb:.1f} MB"
        assert peak_memory_mb < 2000, f"Peak memory too high: {peak_memory_mb:.1f} MB"
        
        # Processing time should be reasonable
        assert monitor['total_time'] < 60, f"Processing time too long: {monitor['total_time']:.2f}s"
