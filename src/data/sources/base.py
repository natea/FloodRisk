...
class BaseDataSource(ABC):
    ...
    def _download_file(
        self, url: str, output_path: Path, resume: bool = True, validate: bool = True
    ) -> Path:
        ...
        try:
            ...
            mode = (
                "ab"
                if resume and temp_path.exists() and response.status_code == 206
                else "wb"
            )
    ...
    def _validate_download(self, file_path: Path, headers: Dict[str, str]) -> None:
        ...
        self.logger.debug(
            f"Download validation passed: {file_path} ({file_size} bytes)"
        )
