...
class USGS3DEPDownloader(BaseDataSource):
    ...
    def download_data(
        ...
        **kwargs
        ...
        if not products:
            raise DataSourceError(f"No DEM products found for specified area and resolution")
        ...
        for i, product in enumerate(products):
            try:
                ...
                self.logger.info(f"Downloading DEM {i+1}/{len(products)}: {product['title']}")
                ...
                self.logger.error(f"Failed to download product {product.get('id', 'unknown')}: {e}")
                continue
    ...
    def download_region(
        ...
        **kwargs
        ...
        if output_dir is None:
            output_dir = self.config.data_dir / "regions" / region_name / "dem" / f"{resolution}m"
    ...
    def get_seamless_dem(
        ...
        if resolution not in [10, 30, 60]:
            raise DataSourceError(f"Seamless service does not support {resolution}m resolution")
        ...
        if output_path is None:
            output_path = (
                self.config.data_dir / "dem" / "seamless" / 
                f"seamless_dem_{resolution}m_{west}_{south}_{east}_{north}.tif"
            )
        
        # Construct seamless service URL
        service_url = f"{self.static_url}/SRTMGL{resolution}/SRTMGL{resolution}_srtm.tif"
        ...
        try:
            ...
            self.logger.warning(f"Seamless service failed, falling back to standard API: {e}")
