...
class NOAAAtlas14Fetcher(BaseDataSource):
    ...
    def get_available_data(
        self, location: Union[Tuple[float, float], Dict[str, float]], **kwargs
    ) -> Dict[str, Any]:
        ...
        cache_key = self._generate_cache_key(lon=lon, lat=lat, action="available_data")
        ...
        self.logger.info(
            f"Querying available NOAA Atlas 14 data for ({lon:.4f}, {lat:.4f})"
        )
    ...
    def download_data(self, **kwargs) -> List[Path]:
        ...
        if "location" in kwargs:
            ...
            raise DataSourceError(
                "Must specify either 'location' for point data or 'bbox'/'region_name' for regional data"
            )
    ...
    def download_point_data(
        ...
        **kwargs,
        ...
        cache_key = self._generate_cache_key(
            lon=lon, lat=lat, return_periods=return_periods, durations=durations_min
        )
        ...
        local_csv_paths = [
            ...
            / "nashville"
            / "rainfall"
            / "All_Depth_English_PDS.csv",
            Path(
                "/Users/nateaune/Documents/code/FloodRisk/data/regions/nashville/rainfall/All_Depth_English_PDS.csv"
            ),
            ...
            try:
                ...
                for rp in return_periods:
                    ...
                    for duration_min in durations_min:
                        ...
                        try:
                            depth_inches = loader.get_precipitation_depth(
                                duration_str, rp
                            )
                            ...
                            self.logger.warning(
                                f"Could not get {duration_str} {rp}-year data: {e}"
                            )
                            # Use approximate value
                            row[f"duration_{duration_min}_min"] = (
                                self._estimate_precipitation(rp, duration_min)
                            )
                ...
                for row in data_rows:
                    ...
                    row["longitude"] = lon
                    row["station"] = (
                        f"{metadata.get('station', 'Nashville WSO Airport')} (interpolated)"
                    )
                    row["source"] = "NOAA Atlas 14 Local CSV"

                df = pd.DataFrame(data_rows)
                self.logger.info(
                    f"Loaded precipitation data from local CSV for {len(return_periods)} return periods"
                )

            except Exception as e:
                self.logger.warning(
                    f"Failed to load local CSV: {e}. Falling back to synthetic data."
                )
                df = self._generate_sample_atlas14_data(
                    return_periods, durations_min, lat, lon
                )
        else:
            self.logger.warning(
                f"No local NOAA Atlas 14 CSV found. Generating sample precipitation data for point ({lon:.4f}, {lat:.4f})"
            )
        ...
        **kwargs,
        ...
        for i, (lon, lat) in enumerate(zip(lon_points, lat_points)):
            if successful_points >= max_points:
                self.logger.info(
                    f"Reached maximum sample points limit ({max_points}), stopping..."
                )
                break

            try:
                self.logger.info(
                    f"Processing grid point {i+1}/{len(lon_points)}: ({lon:.4f}, {lat:.4f})"
                )
                ...
                self.logger.warning(
                    f"Failed to download data for point ({lon:.4f}, {lat:.4f}): {e}"
                )
                continue
        ...
        self.logger.info(
            f"Successfully downloaded {len(downloaded_files)} precipitation data files"
        )
    ...
    def _parse_atlas14_response(
        ...
        for duration_min in durations_min:
            duration_label = self.duration_minutes.get(
                duration_min, f"{duration_min}-min"
            )
            ...
            for return_period in return_periods:
                ...
                if "data" in response_data:
                    ...
                    if data_key in response_data["data"]:
                        record["precipitation_inches"] = response_data["data"][
                            data_key
                        ].get("estimate", 0.0)
                        record["lower_bound"] = response_data["data"][data_key].get(
                            "lower", 0.0
                        )
                        record["upper_bound"] = response_data["data"][data_key].get(
                            "upper", 0.0
                        )
    ...
    def _estimate_precipitation(self, return_period: int, duration_min: int) -> float:
        ...
        base_24hr = {10: 100, 25: 125, 50: 150, 100: 175, 500: 225, 1000: 250}
    ...
    def _generate_sample_atlas14_data(
        ...
        base_24hr_depths = {10: 4.5, 25: 5.2, 50: 5.8, 100: 6.4, 500: 7.8}
            ...
            if duration_min in duration_factors:
                ...
                if duration_min < sorted_durations[0]:
                    ...
                    if lower_dur == upper_dur:
                        ...
                        duration_factor = (
                            duration_factors[lower_dur] * (1 - ratio)
                ...
                if return_period in base_24hr_depths:
                    ...
                    if return_period < sorted_rps[0]:
                        ...
                        if lower_rp == upper_rp:
                            ...
                            log_ratio = (
                                math.log(return_period) - math.log(lower_rp)
                            ) / (math.log(upper_rp) - math.log(lower_rp))
                            base_depth = (
                                base_24hr_depths[lower_rp] * (1 - log_ratio)
    ...
    def download_region(
        ...
        **kwargs,
        ...
        if output_dir is None:
            output_dir = (
                self.config.data_dir / "regions" / region_name / "rainfall" / "atlas14"
            )
