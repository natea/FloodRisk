...
class NOAAAtlas14LocalLoader:
    ...
    def _parse_csv(self) -> Dict:
        ...
        """
        data = {"metadata": {}, "estimates": {}, "upper_bound": {}, "lower_bound": {}}
        ...
        sections = {"estimates": None, "upper_bound": None, "lower_bound": None}

        for i, line in enumerate(lines):
            if (
                "PRECIPITATION FREQUENCY ESTIMATES" in line
                and "UPPER BOUND" not in line
                and "LOWER BOUND" not in line
            ):
        ...
        for section_name, start_idx in sections.items():
            ...
            return_periods = [
                int(x.strip()) for x in header_line.split(":,")[1].split(",")
            ]
            ...
            while i < len(lines):
                ...
                if (
                    not line
                    or line.startswith("Date/time")
                    or line.startswith("pyRunTime")
                ):
                    break
                if ":" not in line:
                    break
        ...
        logger.info(
            f"Parsed {len(data['estimates'])} durations with {len(return_periods)} return periods"
        )

        return data

    def get_precipitation_depth(
        self, duration: str, return_period: int, confidence: str = "estimates"
    ) -> float:
        """
        ...
        if return_period not in self.data[confidence][duration]:
            ...
            raise ValueError(
                f"Return period {return_period} not found. Available: {available}"
            )

        return self.data[confidence][duration][return_period]

    def get_24hr_precipitation(
        self, return_periods: List[int] = None
    ) -> Dict[int, float]:
        """
        ...
        for rp in return_periods:
            try:
                ...
                results[rp] = depth_mm
                logger.info(
                    f"{rp}-year 24-hr precipitation: {depth_inches:.2f} in ({depth_mm:.1f} mm)"
                )
    ...
    def create_rainfall_config(self, output_path: Path = None) -> Dict:
        ...
        for duration in self.data["estimates"]:
            config["precipitation_depths_inches"][duration] = self.data["estimates"][
                duration
            ]
...
def update_nashville_rainfall_config():
    ...
    config_path = Path(
        "/Users/nateaune/Documents/code/FloodRisk/data/nashville/rainfall_config.json"
    )
