...
class DataConfig:
    ...
    return_periods_years: List[int] = field(
        default_factory=lambda: [10, 25, 50, 100, 500]
    )
    ...
    def target_crs_obj(self) -> CRS:
        ...
        return (
            CRS.from_epsg(self.target_crs)
            if isinstance(self.target_crs, int)
            else CRS.from_string(self.target_crs)
        )
