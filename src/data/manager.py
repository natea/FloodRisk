...
class DataManager:
    ...
    def download_all_region_data(
        ...
        if parallel:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                ...
                rainfall_future = executor.submit(
                    ...
                    region_name,
                    rainfall_return_periods,
                    rainfall_durations_hours,
                    rainfall_grid_spacing,
                    output_dir / "rainfall",
                )
                ...
                for future in as_completed([dem_future, rainfall_future]):
                    try:
                        ...
                        self.logger.info(
                            f"Completed {source_type} download: {len(files)} files"
                        )
            ...
            try:
                ...
                self.logger.info(
                    f"Rainfall download completed: {len(results['rainfall'])} files"
                )
            except Exception as e:
        ...
        self.logger.info(
            f"Data download completed for {region_name}: {total_files} total files"
        )

        return results

    def _download_dem_safe(
        self, region_name: str, resolution: int, output_dir: Path
    ) -> tuple[str, List[Path]]:
        """Safely download DEM data with error handling."""
        try:
            files = self.usgs_downloader.download_region(
                region_name=region_name, resolution=resolution, output_dir=output_dir
            )
            return ("dem", files)
        except Exception as e:
            self.logger.error(f"DEM download failed: {e}")
    ...
    def _generate_download_report(
        self, results: Dict[str, List[Path]], report_path: Path
    ) -> None:
    ...
    def validate_data_integrity(
        self, file_paths: List[Path], data_type: str = "unknown"
    ) -> Dict[str, List[Path]]:
