...
class PredictionInput(BaseModel):
    ...
    elevation: Optional[float] = Field(
        None, description="Elevation in meters above sea level"
    )

    # Historical data
    historical_rainfall: Optional[List[float]] = Field(
        None, description="Historical rainfall data in mm"
    )
    historical_water_levels: Optional[List[float]] = Field(
        None, description="Historical water levels in meters"
    )

    # Current conditions
    current_rainfall: Optional[float] = Field(
        None, ge=0, description="Current rainfall in mm"
    )
    current_water_level: Optional[float] = Field(
        None, ge=0, description="Current water level in meters"
    )
    ...
    drainage_capacity: Optional[float] = Field(
        None, ge=0, description="Drainage capacity in cubic meters per second"
    )
    population_density: Optional[float] = Field(
        None, ge=0, description="Population per square kilometer"
    )
    building_density: Optional[float] = Field(
        None, ge=0, le=100, description="Building coverage percentage"
    )

    # Forecast data
    forecast_rainfall: Optional[List[float]] = Field(
        None, description="Forecasted rainfall for next 24-72 hours"
    )
...
class BatchPredictionInput(BaseModel):
    ...
    predictions: List[PredictionInput] = Field(
        ..., max_items=100, description="List of prediction inputs"
    )
    include_confidence: bool = Field(
        True, description="Include confidence scores in response"
    )
...
class PredictionOutput(BaseModel):
    ...
    predicted_water_level: Optional[float] = Field(
        None, description="Predicted peak water level in meters"
    )
    time_to_peak: Optional[int] = Field(
        None, description="Time to peak flood level in hours"
    )
    affected_area: Optional[float] = Field(
        None, description="Estimated affected area in square kilometers"
    )

    # Risk factors
    primary_factors: Optional[List[str]] = Field(
        None, description="Primary contributing risk factors"
    )
    recommendations: Optional[List[str]] = Field(
        None, description="Risk mitigation recommendations"
    )
...
class BatchPredictionOutput(BaseModel):
    ...
    predictions: List[PredictionOutput] = Field(
        ..., description="List of prediction outputs"
    )
...
class ValidationRequest(BaseModel):
    ...
    actual_flood_occurred: bool = Field(
        ..., description="Whether flood actually occurred"
    )
    actual_water_level: Optional[float] = Field(
        None, description="Actual peak water level in meters"
    )
    actual_affected_area: Optional[float] = Field(
        None, description="Actual affected area in square kilometers"
    )
    prediction_id: Optional[str] = Field(
        None, description="Original prediction ID for tracking"
    )
    validation_notes: Optional[str] = Field(
        None, max_length=1000, description="Additional validation notes"
    )
...
class ValidationResponse(BaseModel):
    ...
    accuracy_score: Optional[float] = Field(
        None, description="Prediction accuracy score"
    )
    status: str = Field(..., description="Validation status")
    recorded_timestamp: datetime = Field(
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "version": "1.0.0",
                "model_status": "loaded",
                "dependencies": {"database": "connected", "model": "loaded"},
            }
        }
