...
class DataNormalizer:
    ...
    def z_score_normalize(self, data: np.ndarray, 
                         feature_name: str = 'unnamed',
                         use_robust: bool = False) -> Tuple[np.ndarray, NormalizationParams]:
        ...
        if use_robust:
            ...
            if scale < self.epsilon:
                scale = 1.0
                warnings.warn(f"Very small MAD for feature {feature_name}, using scale=1.0")
            ...
            if std < self.epsilon:
                std = 1.0
                warnings.warn(f"Very small std for feature {feature_name}, using std=1.0")
    ...
    def min_max_normalize(self, data: np.ndarray,
                         feature_name: str = 'unnamed',
                         target_range: Tuple[float, float] = (0, 1),
                         percentile_range: Optional[Tuple[float, float]] = None) -> Tuple[np.ndarray, NormalizationParams]:
    ...
    def local_normalize(self, data: np.ndarray,
                       window_size: int,
                       feature_name: str = 'unnamed',
                       method: str = 'z_score') -> Tuple[np.ndarray, NormalizationParams]:
        ...
        local_mean = uniform_filter(data.astype(np.float64), size=window_size, mode='reflect')
        
        if method == 'z_score':
            # Calculate local standard deviation
            local_var = uniform_filter(data.astype(np.float64)**2, size=window_size, mode='reflect') - local_mean**2
    ...
    def quantile_normalize(self, data: np.ndarray,
                          feature_name: str = 'unnamed',
                          n_quantiles: int = 1000) -> Tuple[np.ndarray, NormalizationParams]:
        ...
        for i in range(len(data.flat)):
            ...
            if np.isnan(value) or (hasattr(data, 'mask') and data.mask.flat[i]):
                ...
                # Find quantile rank
                rank = np.searchsorted(reference_quantiles, value) / len(reference_quantiles)
    ...
    def log_normalize(self, data: np.ndarray,
                     feature_name: str = 'unnamed',
                     offset: Optional[float] = None) -> Tuple[np.ndarray, NormalizationParams]:
    ...
    def power_normalize(self, data: np.ndarray,
                      feature_name: str = 'unnamed',
                      power: Optional[float] = None) -> Tuple[np.ndarray, NormalizationParams]:
    ...
    def compute_dimensionless_features(self, features: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        if 'elevation' in features:
            ...
            if elevation.ndim == 2:
                ...
                # Relative elevation
                local_min = uniform_filter(elevation.astype(np.float64), size=15, mode='minimum')
                local_max = uniform_filter(elevation.astype(np.float64), size=15, mode='maximum')
                local_range = local_max - local_min
                local_range = np.maximum(local_range, self.epsilon)
        ...
        if 'slope' in features and 'elevation' in features:
            ...
            if slope.ndim == 2 and elevation.ndim == 2:
                ...
                slope_position = (slope - local_slope_mean) / (local_slope_mean + self.epsilon)
        ...
        if 'hand' in features and 'elevation' in features:
            ...
            if hand.ndim == 2 and elevation.ndim == 2:
                local_relief = uniform_filter(elevation.astype(np.float64), size=15, mode='maximum') - \
                              uniform_filter(elevation.astype(np.float64), size=15, mode='minimum')
        ...
        if 'flow_accumulation' in features:
            ...
            if flow_acc.ndim == 2:
                ...
                flow_contrast = (log_flow_acc - local_log_mean) / (local_log_mean + self.epsilon)
        ...
        if 'roughness' in features:
            ...
            if roughness.ndim == 2:
                local_roughness_mean = uniform_filter(roughness.astype(np.float64), size=9)
                roughness_contrast = (roughness - local_roughness_mean) / (local_roughness_mean + self.epsilon)
    ...
    def normalize_feature_set(self, features: Dict[str, np.ndarray],
                             normalization_config: Dict[str, str]) -> Dict[str, np.ndarray]:
            ...
            if feature_name in normalization_config:
                ...
                try:
                    if method == 'z_score':
                        ...
                        normalized, _ = self.z_score_normalize(data, feature_name, use_robust=True)
                    elif method == 'min_max':
                        normalized, _ = self.min_max_normalize(data, feature_name)
                    elif method == 'min_max_robust':
                        normalized, _ = self.min_max_normalize(data, feature_name, percentile_range=(5, 95))
                    elif method == 'log':
                        ...
                        normalized, _ = self.local_normalize(data, window_size, feature_name, local_method)
                    else:
                        warnings.warn(f"Unknown normalization method '{method}' for feature '{feature_name}'")
                        normalized = data.copy()
                    
                    normalized_features[feature_name] = normalized
                    
                except Exception as e:
                    warnings.warn(f"Failed to normalize feature '{feature_name}': {str(e)}")
    ...
    def apply_normalization(self, data: np.ndarray, params: NormalizationParams) -> np.ndarray:
    ...
    def load_normalization_params(self, input_path: Union[str, Path]) -> None:
        ...
        for feature_name, params_dict in serializable_params.items():
            self.normalization_params[feature_name] = NormalizationParams(
                ...
                percentiles=tuple(params_dict['percentiles']) if params_dict.get('percentiles') else None,
