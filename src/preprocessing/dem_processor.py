...
class DEMProcessor:
    ...
    def fill_depressions(
        self, elevation: np.ndarray, max_iterations: int = 1000
    ) -> np.ndarray:
        ...
        for _ in range(max_iterations):
            ...
            new_result = np.maximum(result, np.minimum(dilated, filled_elev + 1e-6))
    ...
    def extract_stream_network(
        self, flow_accumulation: np.ndarray, cell_size: float
    ) -> np.ndarray:
    ...
    def condition_dem(
        ...
        if streams is not None:
            ...
            conditioned = np.where(streams, conditioned - stream_depth, conditioned)

        return conditioned

    def process_dem(
        self, dem_path: Union[str, Path], output_path: Optional[Union[str, Path]] = None
    ) -> Dict[str, np.ndarray]:
    ...
    def validate_processing(self, results: Dict[str, np.ndarray]) -> Dict[str, bool]:
        ...
        validation["flow_accumulation_valid"] = (
            np.all(flow_acc >= 1) and np.max(flow_acc) <= flow_acc.size
        )
