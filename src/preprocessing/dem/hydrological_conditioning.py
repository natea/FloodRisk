...
class HydrologicalConditioner:
    ...
    def fill_sinks(self, method: str = 'planchon_darboux', epsilon: float = 0.01) -> np.ndarray:
        """
        ...
        if method == 'planchon_darboux':
            ...
            filled_dem = rd.FillDepressions(rd_dem, method='Wei2018', epsilon=epsilon, in_place=False)
        elif method == 'wei':
    ...
    def calculate_flow_direction(self, filled_dem: np.ndarray, method: str = 'd8') -> np.ndarray:
        """
    ...
    def condition_dem(self, stream_threshold: float = 1000, 
                     buffer_distance: float = 100) -> Dict[str, np.ndarray]:
        """
    ...
    def _condition_along_streams(self, dem: np.ndarray, streams: np.ndarray, 
        ...
        stream_buffer = ndimage.binary_dilation(
            streams, iterations=buffer_pixels
        )
