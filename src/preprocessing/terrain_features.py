...
class TerrainFeatureExtractor:
    ...
    def compute_slope(
        self, elevation: np.ndarray, units: str = "degrees"
    ) -> np.ndarray:
    ...
    def compute_aspect(self, elevation: np.ndarray) -> np.ndarray:
        ...
        aspect_degrees = np.where(
            aspect_degrees < 0, aspect_degrees + 360, aspect_degrees
        )
    ...
    def compute_curvature(self, elevation: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        profile_curvature = np.where(
            small_slope_mask,
            0,
            (fxx * p2 + 2 * fxy * pq + fyy * q2)
            / (denominator * np.sqrt(denominator + 1)),
        )
        ...
        planform_curvature = np.where(
            small_slope_mask,
            0,
            (fxx * q2 - 2 * fxy * pq + fyy * p2)
            / (denominator * np.sqrt(1 + denominator)),
        )
    ...
    def compute_hand(
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                for si, sj, se in zip(
                    stream_coords[0], stream_coords[1], stream_elevations
                ):
                    # Calculate distance
    ...
    def compute_roughness(
        self, elevation: np.ndarray, window_size: int = 3
    ) -> np.ndarray:
    ...
    def validate_features(self, features: Dict[str, np.ndarray]) -> Dict[str, bool]:
        ...
        if "twi" in features:
            twi = features["twi"]
            validation["twi_reasonable"] = np.nanmin(twi) > -20 and np.nanmax(twi) < 30
