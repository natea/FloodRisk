...
class TerrainFeatureExtractor:
    ...
    def __init__(self, dem_array: np.ndarray, pixel_size: float, 
    ...
    def calculate_slope(self, method: str = 'horn', units: str = 'degrees') -> np.ndarray:
        """
    ...
    def calculate_roughness(self, window_size: int = 3) -> np.ndarray:
        ...
        def local_std(values):
            return np.std(values[~np.isnan(values)]) if np.any(~np.isnan(values)) else np.nan
    ...
    def calculate_flow_accumulation(self, flow_direction: Optional[np.ndarray] = None) -> np.ndarray:
        """
    ...
    def calculate_wetness_index(self, flow_accumulation: np.ndarray, 
    ...
    def extract_all_features(self, flow_direction: Optional[np.ndarray] = None) -> Dict[str, np.ndarray]:
        """
...
class HANDCalculator:
    ...
    def __init__(self, dem: np.ndarray, flow_accumulation: np.ndarray, 
                 pixel_size: float, stream_threshold: float = 1000):
        """
    ...
    def _find_nearest_stream(self, row: int, col: int, max_pixels: int) -> Tuple[Optional[int], Optional[int]]:
        ...
        for distance in range(1, max_pixels + 1):
            ...
            for dr in range(-distance, distance + 1):
                for dc in range(-distance, distance + 1):
                    ...
                    # Check bounds
                    if (0 <= r < self.streams.shape[0] and 
                        0 <= c < self.streams.shape[1] and
                        self.streams[r, c]):
...
def calculate_terrain_derivatives(dem: np.ndarray, pixel_size: float) -> Dict[str, np.ndarray]:
    """
    ...
    # Mean curvature
    mean_curv = (r * (1 + q**2) - 2*p*q*s + t * (1 + p**2)) / (2 * (1 + p**2 + q**2)**(3/2))
