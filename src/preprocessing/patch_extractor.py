...
class PatchExtractor:
    ...
    def __init__(
        self,
        patch_sizes_m: List[int] = [256, 512, 1024],
    ...
    def _is_patch_valid(
        self, data: np.ndarray, nodata_value: Optional[float] = None
    ) -> bool:
    ...
    def _generate_patch_grid(
        self, raster_height: int, raster_width: int, patch_size_pixels: int
    ) -> List[Tuple[int, int]]:
    ...
    def _create_patch_info(
        ...
        patch_transform = from_bounds(
            minx, miny, maxx, maxy, patch_size_pixels, patch_size_pixels
        )
    ...
    def extract_patches_from_file(
        ...
        with rasterio.open(raster_path) as src:
            ...
            if patch_size_pixels >= min(height, width):
                warnings.warn(
                    f"Patch size ({patch_size_pixels}px) is larger than raster dimensions ({height}x{width}px)"
                )
                return patches
            ...
            for i, (row, col) in enumerate(positions):
                ...
                try:
                    ...
                    warnings.warn(
                        f"Failed to extract patch at ({row}, {col}): {str(e)}"
                    )
                    continue
    ...
    def extract_patches_batch(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(process_file, path): path for path in raster_paths
            }
    ...
    def filter_patches(
        ...
        max_data_value: Optional[float] = None,
    ) -> List[Dict[str, Any]]:
