...
class RainfallGenerator:
    ...
    def generate_design_storm(
        ...
        distribution_type: str = "scs_type_ii",
    ) -> Dict[str, np.ndarray]:
    ...
    def generate_noaa_atlas14_scenarios(
        self,
        precipitation_data: Dict[int, Dict[int, float]],
        ...
        for rp in return_periods:
            ...
            for duration in durations_to_use:
                if duration not in rp_data:
                    warnings.warn(
                        f"Duration {duration}h not available for {rp}-year return period"
                    )
                    continue
    ...
    def generate_multiple_storm_ensemble(
        self,
        base_scenario: Dict[str, np.ndarray],
        num_realizations: int = 100,
        intensity_variability: float = 0.2,
        timing_variability: float = 0.1,
        ...
        for i in range(num_realizations):
            ...
            if timing_variability > 0:
                ...
                time_shift = np.clip(
                    time_shift, -duration / 4, duration / 4
                )  # Limit shift
    ...
    def save_scenarios(
        self, scenarios: Dict[str, Dict], output_path: Union[str, Path]
    ) -> None:
    ...
    def load_noaa_atlas14_data(
        self, data_file: Union[str, Path]
