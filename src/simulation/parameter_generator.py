...
class ParameterFileGenerator:
    ...
    def __init__(self, 
        ...
        logger.info(f"Parameter generator initialized with base config: {self.base_config_dir}")
    
    def generate_scenario_parameters(self,
                                   dem_file: str,
                                   return_periods: List[ReturnPeriodConfig],
                                   hyetograph_patterns: List[HyetographConfig],
                                   output_dir: str,
                                   base_simulation_config: Optional[Dict] = None) -> List[Dict]:
        ...
        for rp_config in return_periods:
            for hyet_config in hyetograph_patterns:
                ...
                par_file = self._generate_parameter_file(
                    par_config,
                    output_path / f"simulation_{scenario_id}.par"
                )
    ...
    def _create_scenario_id(self, 
                          ...
                          hyet_config: HyetographConfig) -> str:
    ...
    def _generate_rainfall_file(self,
                              hyet_config: HyetographConfig,
        ...
        logger.debug(f"Total depth: {np.sum(hourly_intensities):.2f}mm, "
                    f"Peak intensity: {np.max(hourly_intensities):.2f}mm/h")
        
        return str(output_file)
    
    def _create_hyetograph_pattern(self, 
                                 config: HyetographConfig, 
                                 total_depth_mm: float) -> np.ndarray:
