...
class ResourceManager:
    ...
    def __init__(self, config: OptimizedBatchConfig):
        ...
        self.available_memory_gb = min(config.max_memory_gb, 
        ...
        logger.info(f"Resource manager initialized: {self.available_cpus} CPUs, "
                   f"{self.available_memory_gb:.1f}GB RAM")
    
    def can_allocate_resources(self, cpu_req: int, memory_gb_req: float) -> bool:
        """Check if resources can be allocated."""
        with self.resource_lock:
            return (self.allocated_cpus + cpu_req <= self.available_cpus and
                   self.allocated_memory_gb + memory_gb_req <= self.available_memory_gb)
    
    def allocate_resources(self, job_id: str, cpu_req: int, memory_gb_req: float) -> bool:
        ...
        with self.resource_lock:
            if self.can_allocate_resources(cpu_req, memory_gb_req):
                ...
                self.job_resources[job_id] = {'cpu': cpu_req, 'memory_gb': memory_gb_req}
                
                logger.debug(f"Allocated resources for {job_id}: {cpu_req} CPUs, {memory_gb_req}GB")
                return True
            return False
    ...
    def get_resource_utilization(self) -> Dict[str, float]:
        ...
        with self.resource_lock:
            return {
                ...
                'memory_utilization': self.allocated_memory_gb / self.available_memory_gb,
...
class JobScheduler:
    ...
    def _adaptive_scheduling(self, max_jobs: int) -> List[Dict[str, Any]]:
        ...
        # Sort by priority
        sorted_jobs = sorted(self.pending_jobs, key=lambda x: x['priority'], reverse=True)
        
        for job_spec in sorted_jobs[:max_jobs]:
            # Check resource requirements
            cpu_req = job_spec.get('cpu_requirement', self.config.cpu_limit_per_job)
            memory_req = job_spec.get('memory_requirement', self.config.memory_limit_per_job_gb)
...
class OptimizedSimulationBatch:
    ...
    def __init__(self,
        ...
        logger.info(f"OptimizedSimulationBatch initialized with {self.config.max_parallel_jobs} max jobs")
    ...
    def _optimize_scenario_execution_plan(self):
        ...
        if self.config.scheduler_type == "adaptive":
            ...
            self.scenarios.sort(key=lambda x: x['estimated_cpu_time'] / max(x['estimated_memory_gb'], 0.1))
    ...
    async def execute_batch_async(self) -> Dict:
        ...
        try:
            ...
            logger.info(f"Optimized batch execution completed: "
    ...
    async def _execute_scenarios_optimized(self) -> List[Dict]:
        ...
        while (self.scheduler.pending_jobs or active_futures or 
    ...
    async def _monitor_performance(self):
        ...
        while True:
            try:
                ...
                logger.debug(f"System: CPU={cpu_percent:.1f}%, Memory={memory.percent:.1f}%, "
    ...
    def _generate_optimized_summary(self) -> Dict:
        ...
        summary = {
            ...
            'throughput_scenarios_per_hour': len(successful_results) / (runtime_seconds / 3600) if runtime_seconds > 0 else 0,
...
def create_optimized_batch_orchestrator(simulator: LisfloodSimulator,
                                      parameter_generator: ParameterFileGenerator,
                                      **config_kwargs) -> OptimizedSimulationBatch:
    ...
    logger.info(f"Created optimized batch orchestrator with {config.max_parallel_jobs} max jobs")
    return orchestrator