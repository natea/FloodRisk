...
class PreprocessingIntegration:
    ...
    def prepare_dem_for_simulation(
        self,
        preprocessed_dem_path: str,
        output_path: Optional[str] = None,
        target_format: str = "asc",
    ) -> str:
    ...
    def _convert_dem_with_rasterio(
    ...
    def create_auxiliary_input_files(
        self,
    ...
    def _create_constant_ascii_grid(
        self,
        value: float,
        shape: Tuple[int, int],
    ...
    def extract_rainfall_from_preprocessing(
        self,
        ...
        if "rainfall_statistics" in preprocessing_metadata:
            ...
            for rp in return_periods:
                ...
                for key in possible_keys:
                    ...
                    logger.warning(
                        f"No rainfall data found for {rp}-year return period"
                    )
        ...
        for rp in return_periods:
            if rp not in rainfall_data and rp in defaults:
                ...
                logger.info(
                    f"Using default rainfall for {rp}-year: {defaults[rp]:.1f} mm"
                )
    ...
    def setup_simulation_from_preprocessing(
        ...
        simulation_dem = self.prepare_dem_for_simulation(
            preprocessed_dem, output_path=self.temp_dir / "simulation_dem.asc"
        )

        # Create auxiliary files
        auxiliary_files = self.create_auxiliary_input_files(
            dem_bounds=bounds, grid_shape=grid_shape, output_dir=str(self.temp_dir)
        )
        ...
        rainfall_depths = self.extract_rainfall_from_preprocessing(
            preprocess_config.get("metadata", {}), return_periods, location_name
        )
        ...
        for pattern in hyetograph_patterns:
            if pattern == "uniform":
                ...
                hyet_configs.append(
                    HyetographConfig("front_loaded", 0, front_factor=2.5)
                )
            elif pattern == "back_loaded":
                hyet_configs.append(
                    HyetographConfig("back_loaded", 0, front_factor=2.5)
                )
        ...
        logger.info(
            f"  - Return periods: {[rp.return_period_years for rp in rp_configs]}"
        )
    ...
    def run_integrated_simulation(
        ...
        setup_info = self.setup_simulation_from_preprocessing(
            preprocessing_config_file, return_periods, hyetograph_patterns
        )
    ...
    def export_for_ml_training(
        ...
        ml_training_dir: str,
        include_preprocessing_features: bool = True,
    ) -> str:
        ...
        for result in successful:
            ...
            if include_preprocessing_features:
                sample["preprocessing_features"] = {
                    "dem_file": setup_info["dem_file"],
                    "preprocessing_source": setup_info.get(
