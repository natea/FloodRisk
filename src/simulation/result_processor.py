...
class ResultProcessor:
    ...
    def process_simulation_output(self,
                                ...
                                output_dir: str,
                                simulation_id: str,
                                dem_bounds: Optional[Tuple[float, float, float, float]] = None) -> Dict:
        ...
        try:
            ...
            qc_results = self._quality_control_checks(depth_data, flood_extent, statistics)
            ...
            logger.info(f"Successfully processed {simulation_id}: "
    ...
    def process_batch_outputs(self,
        ...
        for sim_result in simulation_results:
            if sim_result.get('status') != 'success':
                logger.warning(f"Skipping failed simulation: {sim_result.get('simulation_id', 'unknown')}")
                continue
            ...
            if not depth_file or not Path(depth_file).exists():
                logger.warning(f"Depth file not found for {simulation_id}: {depth_file}")
                failed_count += 1
                continue
        ...
        logger.info(f"Batch processing completed: {len(successful_results)}/{len(processed_results)} successful")
    ...
    def _load_depth_data(self, file_path: str) -> np.ndarray:
        ...
        try:
            ...
            depth_array = np.where(depth_array < 0, 0, depth_array)  # Remove negative depths
            depth_array = np.where(np.isnan(depth_array), 0, depth_array)  # Remove NaN values
            
            logger.debug(f"Loaded depth data: shape={depth_array.shape}, "
    ...
    def _calculate_statistics(self, depth_data: np.ndarray, flood_extent: np.ndarray) -> Dict:
        ...
        if flooded_pixels > 0:
            ...
            depth_percentiles = np.percentile(depth_data[flood_extent > 0], [50, 75, 90, 95, 99])
        else:
        ...
        statistics = {
            ...
            'flooded_area_km2': float(flooded_pixels * (self.config.pixel_size_m ** 2) / 1e6)
        }
        
        return statistics
    
    def _quality_control_checks(self, depth_data: np.ndarray, flood_extent: np.ndarray, statistics: Dict) -> Dict:
        ...
        if flood_fraction < min_expected:
            qc['warnings'].append(f"Low flood fraction: {flood_fraction:.4%} < {min_expected:.4%}")
        elif flood_fraction > max_expected:
            qc['errors'].append(f"High flood fraction: {flood_fraction:.4%} > {max_expected:.4%}")
        
        # Check for unrealistic depths
        if max_depth > self.config.max_depth_threshold_m:
            qc['errors'].append(f"Unrealistic max depth: {max_depth:.2f}m > {self.config.max_depth_threshold_m}m")
    ...
    def _save_outputs(self,
                     ...
                     output_path: Path,
                     simulation_id: str,
                     dem_bounds: Optional[Tuple[float, float, float, float]]) -> Dict:
    ...
    def _save_geotiff(self, data: np.ndarray, output_file: Path, bounds: Tuple[float, float, float, float]):
    ...
    def _save_batch_results(self, results: List[Dict], summary: Dict, output_path: Path):
    ...
    def _create_training_manifest(self, successful_results: List[Dict]) -> Dict:
        ...
        for result in successful_results:
            sample = {
                ...
                'flood_extent_file': result['output_files'].get('extent_numpy') or result['output_files'].get('extent_geotiff'),
                'depth_file': result['output_files'].get('depth_numpy') or result['output_files'].get('depth_geotiff'),
                'statistics': result['statistics'],
                'return_period': result.get('original_simulation', {}).get('scenario', {}).get('return_period', {}),
                'hyetograph': result.get('original_simulation', {}).get('scenario', {}).get('hyetograph', {})
            }
