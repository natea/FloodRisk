...
class SimulationValidator:
    ...
    def validate_single_simulation(self,
                                 simulation_result: Dict,
                                 depth_data: Optional[np.ndarray] = None,
                                 flood_extent: Optional[np.ndarray] = None) -> ValidationResult:
        ...
        try:
            ...
            if simulation_result.get('status') != 'success':
                ...
                result.errors.append(f"Simulation failed: {simulation_result.get('error', 'Unknown error')}")
                return result
            ...
            # Physical validation
            result.physical_checks = self._validate_physical_realism(statistics, depth_data)
            
            # Spatial validation
            result.spatial_checks = self._validate_spatial_characteristics(statistics, flood_extent)
            
            # Temporal validation
            result.temporal_checks = self._validate_temporal_characteristics(simulation_result)
            
            # Data quality validation
            result.data_quality_checks = self._validate_data_quality(statistics, depth_data)
            
            # Compile overall assessment
            result = self._compile_validation_result(result)
            
            logger.info(f"Validation completed for {simulation_id}: {result.status} (score: {result.overall_score:.1f})")
    ...
    def validate_batch_results(self,
                             ...
                             detailed_validation: bool = True) -> Dict:
        ...
        logger.info(f"Batch validation completed: {batch_summary['summary']['passed_count']}/{len(simulation_results)} passed")
        
        return batch_summary
    
    def _validate_physical_realism(self,
                                 statistics: Dict,
        ...
        if max_depth < self.thresholds.min_reasonable_depth_m:
            ...
            checks['depth_range_check']['details']['issue'] = f"Very shallow max depth: {max_depth:.3f}m"
        elif max_depth > self.thresholds.max_reasonable_depth_m:
            checks['depth_range_check']['status'] = 'failed'
            checks['depth_range_check']['details']['issue'] = f"Unrealistic max depth: {max_depth:.1f}m"
        else:
        ...
        if depth_data is not None:
            self._validate_depth_distribution(depth_data, checks['depth_distribution_check'])
        else:
            checks['depth_distribution_check']['status'] = 'skipped'
            checks['depth_distribution_check']['details'] = {'reason': 'No depth data provided'}
        ...
        if flood_fraction > 0.4:  # More than 40% flooded suggests issues
            ...
            checks['mass_conservation_check']['details']['issue'] = f"High flood fraction: {flood_fraction:.2%}"
        elif flood_fraction < 0.0001:  # Less than 0.01% suggests no flooding
            checks['mass_conservation_check']['status'] = 'warning'
            checks['mass_conservation_check']['details']['issue'] = f"Very low flood fraction: {flood_fraction:.4%}"
        else:
    ...
    def _validate_depth_distribution(self, depth_data: np.ndarray, check_result: Dict):
        ...
        try:
            ...
            # Statistical checks
            percentiles = np.percentile(depth_data[depth_data > 0], [50, 90, 95, 99]) if np.any(depth_data > 0) else np.zeros(4)
            
            # Check for reasonable distribution
            if len(percentiles) >= 2 and percentiles[1] > percentiles[0] * 100:  # P90 >> P50
            ...
            if issues:
                check_result['status'] = 'failed' if any(['NaN' in issue or 'Infinite' in issue for issue in issues]) else 'warning'
    ...
    def _validate_spatial_characteristics(self,
                                        statistics: Dict,
                                        flood_extent: Optional[np.ndarray] = None) -> Dict:
        ...
        if flood_fraction < self.thresholds.min_flood_fraction:
            ...
            checks['flood_fraction_check']['details']['issue'] = f"Low flood fraction: {flood_fraction:.4%}"
        elif flood_fraction > self.thresholds.max_flood_fraction:
            checks['flood_fraction_check']['status'] = 'failed'
            checks['flood_fraction_check']['details']['issue'] = f"Excessive flood fraction: {flood_fraction:.2%}"
        else:
        ...
        if flooded_area < self.thresholds.min_flooded_area_m2:
            ...
            checks['flood_area_check']['details']['issue'] = f"Small flooded area: {flooded_area/1e6:.3f} km²"
        elif flooded_area > self.thresholds.max_flooded_area_m2:
            checks['flood_area_check']['status'] = 'failed' 
            checks['flood_area_check']['details']['issue'] = f"Excessive flooded area: {flooded_area/1e6:.1f} km²"
        else:
        ...
        if flood_extent is not None:
            self._validate_spatial_continuity(flood_extent, checks['spatial_continuity_check'])
        else:
            checks['spatial_continuity_check']['status'] = 'skipped'
            checks['spatial_continuity_check']['details'] = {'reason': 'No flood extent data provided'}
        
        return checks
    
    def _validate_spatial_continuity(self, flood_extent: np.ndarray, check_result: Dict):
        ...
        try:
            ...
            connectivity_ratio = largest_component / total_flooded if total_flooded > 0 else 0
            
            # Check for excessive fragmentation
            small_components = sum(1 for size in component_sizes if size < 10)  # Less than 10 pixels
            
            issues = []
            
            if connectivity_ratio < 0.5 and num_components > 20:
                issues.append(f"Highly fragmented flooding: {num_components} components")
            
            if small_components > num_components * 0.8:
                issues.append(f"Many small isolated areas: {small_components}/{num_components}")
            ...
            check_result['details'] = {'reason': 'scipy not available for spatial analysis'}
        except Exception as e:
            check_result['status'] = 'failed'
            check_result['details'] = {'error': str(e)}
    ...
    def _validate_temporal_characteristics(self, simulation_result: Dict) -> Dict:
        ...
        if runtime < self.thresholds.min_simulation_time_s:
            ...
            checks['runtime_check']['details']['issue'] = f"Very fast simulation: {runtime:.1f}s"
        elif runtime > self.thresholds.max_simulation_time_s:
            checks['runtime_check']['status'] = 'warning'
            checks['runtime_check']['details']['issue'] = f"Long simulation: {runtime/3600:.1f}h"
        else:
        ...
        checks['convergence_check']['details'] = {'reason': 'Convergence data not available'}
    ...
    def _validate_data_quality(self,
        ...
        # Completeness check
        required_stats = ['total_pixels', 'flooded_pixels', 'max_depth_m', 'flood_fraction']
        missing_stats = [stat for stat in required_stats if stat not in statistics]
        
        if missing_stats:
            checks['completeness_check']['status'] = 'failed'
            checks['completeness_check']['details']['issue'] = f"Missing statistics: {missing_stats}"
        else:
        ...
        if fraction_difference > 0.001:  # 0.1% tolerance
            ...
            checks['consistency_check']['details']['issue'] = f"Inconsistent flood fraction: calculated={calculated_fraction:.4f}, reported={reported_fraction:.4f}"
        else:
    ...
    def _compile_validation_result(self, result: ValidationResult) -> ValidationResult:
        ...
        for check_group in all_checks:
            for check_name, check_data in check_group.items():
                ...
                if status == 'failed':
                    error_msg = details.get('issue') or details.get('error') or f"{check_name} failed"
        ...
        if total_checks > 0:
            result.overall_score = (passed_checks + 0.5 * warning_checks) / total_checks * 100
        
        return result
    
    def _create_batch_validation_summary(self,
        ...
        summary = {
            'validation_info': {
                ...
                'validator_version': individual_results[0].validator_version if individual_results else "unknown"
            },
            'summary': {
                ...
                'success_rate': passed_count / len(individual_results) if individual_results else 0,
                'average_score': avg_score
            },
            'quality_assessment': {
                'meets_target_success_rate': (passed_count / len(individual_results) if individual_results else 0) >= self.thresholds.target_success_rate,
                'common_errors': dict(sorted(error_frequency.items(), key=lambda x: x[1], reverse=True)),
                'common_warnings': dict(sorted(warning_frequency.items(), key=lambda x: x[1], reverse=True))
            },
            'individual_results': [
                {
    ...
    def _load_depth_data_if_available(self, simulation_result: Dict) -> Optional[np.ndarray]:
    ...
    def _load_extent_data_if_available(self, simulation_result: Dict) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if extent_file and Path(extent_file).exists() and extent_file.endswith('.npy'):
