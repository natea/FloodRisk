...
class SimulationBatch:
    ...
    def create_batch_from_config(
        ...
        output_dir: str,
        base_sim_config: Optional[Dict] = None,
    ) -> str:
        ...
        batch_metadata = {
            ...
            "config": {
                k: v for k, v in self.config.__dict__.items() if not callable(v)
            },
        }
        ...
        logger.info(
            f"Created batch {self.batch_id} with {len(self.scenarios)} scenarios"
        )
        return self.batch_id

    def create_batch_from_scenarios(
        self, scenarios: List[Dict], output_dir: str
    ) -> str:
    ...
    def execute_batch(self) -> Dict:
        ...
        if not self.scenarios:
            raise ValueError(
                "No scenarios defined. Call create_batch_from_config first."
            )
        ...
        logger.info(
            f"Batch execution completed: {summary['success_count']}/{summary['total_scenarios']} successful"
        )
    ...
    def _execute_parallel(self) -> List[Dict]:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            # Submit all jobs
            future_to_scenario = {
                executor.submit(self._execute_scenario_with_retry, scenario): scenario
            ...
            for future in concurrent.futures.as_completed(future_to_scenario):
                ...
                try:
                    ...
                    logger.error(
                        f"Scenario {scenario['scenario_id']} failed with exception: {e}"
                    )
    ...
    def _execute_sequential(self) -> List[Dict]:
        ...
        for i, scenario in enumerate(self.scenarios):
            logger.info(
                f"Executing scenario {i+1}/{len(self.scenarios)}: {scenario['scenario_id']}"
            )
    ...
    def _execute_scenario_with_retry(self, scenario: Dict) -> Dict:
            ...
            try:
                ...
                if self.config.validate_results and result["status"] == "success":
                    ...
                    if not validation["passed"]:
                        logger.warning(
                            f"Scenario {scenario_id} failed validation: {validation['issues']}"
                        )
                ...
                last_error = e
                logger.warning(
                    f"Scenario {scenario_id} attempt {attempt + 1} failed: {e}"
                )
        ...
        # All retries failed
        logger.error(
            f"Scenario {scenario_id} failed after {self.config.max_retries + 1} attempts"
        )
        raise last_error
    ...
    def _generate_batch_summary(self) -> Dict:
        ...
        summary = {
            ...
            "failed_scenarios": [
                r.get("scenario_id", "unknown") for r in failed_results
            ],
    ...
    def export_training_data_paths(self, output_file: str) -> Dict:
        ...
        for result in successful_results:
            ...
            scenario_id = result.get(
                "scenario_id", result.get("simulation_id", "unknown")
            )
        ...
        logger.info(
            f"Exported {len(training_data)} training data paths to {output_file}"
        )
