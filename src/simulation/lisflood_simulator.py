...
class LisfloodSimulator:
    ...
    def __init__(
        ...
        logger.info(
            f"LISFLOOD-FP simulator initialized with executable: {self.lisflood_exe}"
        )
    ...
    def _find_executable(self, exe_path: Optional[str]) -> str:
        ...
        try:
            result = subprocess.run(
                ["which", "lisflood"], capture_output=True, text=True
            )
    ...
    def run_simulation(
        self, config: SimulationConfig, simulation_id: str, cleanup_temp: bool = True
    ) -> Dict:
    ...
    def _execute_lisflood(
        self, par_file: str, sim_dir: Path, max_hours: float
        ...
        try:
            ...
            raise TimeoutError(
                f"Simulation exceeded maximum runtime of {max_hours} hours"
            )

    def _process_outputs(
        self, config: SimulationConfig, sim_dir: Path, simulation_id: str
    ) -> Dict:
        ...
        logger.info(
            f"Processed simulation outputs: {flooded_cells}/{total_cells} cells flooded "
    ...
    def _create_flood_extent(
        self, depth_data: np.ndarray, threshold: float
    ) -> np.ndarray:
        ...
        flood_extent = ndimage.binary_closing(
            flood_extent, structure=struct, iterations=2
        )

        return flood_extent

    def _validate_simulation_results(
        self, output_info: Dict, config: SimulationConfig, runtime_seconds: float
    ) -> Dict:
        ...
        if max_depth > 50.0:  # 50m seems excessive for pluvial flooding
            ...
            validation["warnings"].append(
                f"Very shallow maximum depth: {max_depth:.3f}m"
            )
