...
class ResourceManager:
    ...
    def _should_trigger_cleanup(self, snapshot: ResourceSnapshot) -> bool:
        ...
        if self.memory_limit_bytes:
            memory_usage_ratio = (
                snapshot.memory_used_gb * 1024**3
    ...
    def get_usage_summary(self) -> Dict[str, Any]:
        ...
        return {
            "monitoring_active": self.monitoring,
            "monitoring_duration_minutes": len(self.snapshots)
    ...
    def check_available_resources(self) -> Dict[str, bool]:
        ...
        if snapshot.disk_available_gb < 1.0:  # Less than 1GB available
            ...
            status["disk_message"] = (
                f"Low disk space: {snapshot.disk_available_gb:.1f}GB available"
            )

        status["overall_status"] = (
            status["memory_available"] and status["disk_available"]
        )

        return status

    def estimate_pipeline_resource_needs(
        self, num_simulations: int, dem_size_mb: float, enable_ml_training: bool = True
    ) -> Dict[str, float]:
        ...
        total_disk_gb = (
            dem_products_gb + total_sim_outputs_gb + intermediate_gb + ml_disk_gb
        ) * 1.2
