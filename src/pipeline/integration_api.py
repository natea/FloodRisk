...
class IntegratedFloodPipeline:
    ...
    def __init__(self, 
        ...
        logger.info(f"IntegratedFloodPipeline initialized for {self.config.region_name}")
    ...
    def add_stage_callback(self, stage: str, callback: Callable[[Dict[str, Any]], None]):
        ...
        try:
            ...
            if self.enable_resource_management:
                ...
                if resource_status["overall_status"]:
                    checks["system_resources"] = {"status": "pass", "details": resource_status}
                else:
                    checks["system_resources"] = {"status": "fail", "details": resource_status}
            else:
                checks["system_resources"] = {"status": "skip", "reason": "Resource management disabled"}
            
            # Check Python dependencies
            missing_deps = self._check_dependencies()
            if not missing_deps:
                checks["dependencies"] = {"status": "pass", "details": "All required packages available"}
            else:
                checks["dependencies"] = {
                    "status": "fail", 
    ...
    def _check_dependencies(self) -> List[str]:
        ...
        required_packages = [
            'numpy', 'pandas', 'xarray', 'rasterio', 'geopandas',
            'requests', 'aiohttp', 'matplotlib', 'psutil',
            'torch', 'pytorch_lightning', 'segmentation_models_pytorch'
        ]
    ...
    def _check_data_access(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if usgs_ok and noaa_ok:
                return {
                    "status": "pass",
                    "details": "Data sources accessible"
                }
            else:
                return {
                    "status": "warning", 
    ...
    async def _initialize_components(self):
        ...
        if self.enable_monitoring:
            self.progress_tracker = ProgressTracker(
                self.controller.output_dir / "progress.json",
    ...
    async def _handle_pipeline_error(self, error: Exception):
        ...
        for callback in self.error_callbacks:
            try:
                callback(self.controller.current_stage.value if self.controller else "unknown", error)
    ...
    async def _generate_final_report(self) -> Dict[str, Any]:
        ...
        if self.progress_tracker:
            ...
            report["performance_metrics"] = self.progress_tracker.get_performance_metrics()
        
        if self.resource_manager:
            report["resource_usage"] = self.resource_manager.get_usage_summary()
        
        if self.checkpoint_manager:
            report["checkpoint_summary"] = self.checkpoint_manager.get_checkpoint_summary()
    ...
    def _inventory_outputs(self, output_dir: Path) -> Dict[str, Any]:
        ...
        for file_path in output_dir.rglob("*"):
            if file_path.is_file():
                ...
                if file_path.name in ["final_report.json", "pipeline_metadata.json", "batch_summary.json"]:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self.resource_manager:
            status["resource_usage"] = self.resource_manager.get_current_usage().to_dict()
...
def create_pipeline_from_config(config_file: Union[str, Path]) -> IntegratedFloodPipeline:
