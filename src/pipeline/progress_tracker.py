...
class ProgressTracker:
    ...
    def __init__(
        self, log_file: Optional[Path] = None, auto_save_interval: float = 10.0
    ):
    ...
    def update_stage_progress(
        self, stage_name: str, completion: float, operation: str = ""
    ):
    ...
    def _update_metrics(self):
        ...
        completed_stages = sum(
            1 for stage in self.stages.values() if stage.status == "completed"
        )
        running_stages = [
            stage for stage in self.stages.values() if stage.status == "running"
        ]
        ...
        if running_stages:
            running_progress = sum(
                stage.completion_percentage for stage in running_stages
            )
        ...
        if self.pipeline_start_time and self.metrics.overall_progress > 0:
            ...
            if (
                self.metrics.overall_progress > 0.1
            ):  # Only estimate after 10% completion
                estimated_total_minutes = (
                    elapsed_minutes / self.metrics.overall_progress
                )
                    ...
                    timeline.append(
                        {
                            ...
                            "error": (
                                stage.error_message if stage.error_message else None
                            ),
                        }
                    )
            return timeline
        ...
        with self._lock:
            ...
            return {
                ...
                "stages_by_duration": [
                    ...
                    for s in sorted(
                        completed_stages, key=lambda x: x.duration_seconds, reverse=True
                    )
                ],
            }
    ...
    def _start_auto_save(self):
        ...
        if self.auto_save_interval > 0:
            ...
            self._save_timer = threading.Timer(
                self.auto_save_interval, self._start_auto_save
            )
    ...
    def print_progress_summary(self):
        ...
        with self._lock:
            ...
            print(
                f"Completed Stages: {self.metrics.completed_stages}/{self.metrics.total_stages}"
            )
            print(f"Current Stage: {self.metrics.current_stage}")

            if self.metrics.estimated_remaining_minutes is not None:
                print(
                    f"Estimated Remaining: {self.metrics.estimated_remaining_minutes:.1f} minutes"
                )

            if self.metrics.total_runtime_seconds > 0:
                print(
                    f"Total Runtime: {self.metrics.total_runtime_seconds/60:.1f} minutes"
                )
