...
class CheckpointManager:
    ...
    def get_latest_checkpoint(
        ...
        valid_checkpoints = [
            ...
            if metadata.valid
            and (pipeline_id is None or metadata.pipeline_id == pipeline_id)
        ]
    ...
    def cleanup_old_checkpoints(
        self, keep_count: int = 10, max_age_days: int = 30
    ) -> Dict[str, int]:
        ...
        logger.info(
            f"Cleanup completed: deleted {deleted_count} checkpoints, "
    ...
    def save_error_state(self, stage: PipelineStage, error: str, pipeline_id: str):
        ...
        try:
            checkpoint_id = self.save_checkpoint(
                error_state, f"Error state - {error[:50]}..."
            )
    ...
    def get_recovery_options(self, pipeline_id: str) -> List[Dict[str, Any]]:
        ...
        pipeline_checkpoints.sort(
            key=lambda x: (stage_order.index(x.stage), x.timestamp), reverse=True
        )
        ...
        for cp in pipeline_checkpoints:
            ...
            option = {
                ...
                "estimated_time_saved_percent": (current_stage_idx / len(stage_order))
                * 100,
            }
    ...
    def create_recovery_plan(self, checkpoint_id: str) -> Dict[str, Any]:
        ...
        recovery_plan = {
            ...
            "estimated_completion_percentage": (current_stage_idx / len(stage_order))
            * 100,
    ...
    def get_checkpoint_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "oldest_checkpoint": min(
                (cp.timestamp for cp in self.checkpoints.values()), default=None
            ),
