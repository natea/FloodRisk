...
class PipelineConfig:
    ...
    rainfall_patterns: List[str] = field(
        default_factory=lambda: ["scs_type_ii", "uniform"]
    )
...
class PipelineController:
    ...
    def initialize_components(self):
        ...
        self._dem_processor = DEMProcessor(
            target_crs=self.config.target_crs, resolution=self.config.dem_resolution
        )
    ...
    async def execute_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stages = [
                ...
                (
                    PipelineStage.SIMULATION_VALIDATION,
                    self._stage_simulation_validation,
                ),
            ]
            ...
            if self.config.ml_enabled:
                stages.extend(
                    [
                        (
                            PipelineStage.ML_TRAINING_SETUP,
                            self._stage_ml_training_setup,
                        ),
                        (
                            PipelineStage.ML_TRAINING_EXECUTION,
                            self._stage_ml_training_execution,
                        ),
                    ]
                )
            ...
            logger.info(
                f"Pipeline execution completed successfully in {runtime_minutes:.1f} minutes"
            )
            return summary
            ...
            logger.error(
                f"Pipeline execution failed at stage {self.current_stage.value}: {e}"
            )

            # Save error state for recovery
            if self.checkpoint_manager:
                self.checkpoint_manager.save_error_state(
                    stage=self.current_stage, error=str(e), pipeline_id=self.pipeline_id
                )
    ...
    async def _execute_stage(self, stage: PipelineStage, stage_func):
        ...
        try:
            ...
            self.progress_tracker.complete_stage(
                stage.value, duration_seconds=stage_duration, success=True
            )
    ...
    async def _preprocess_rainfall(
        self, rainfall_data: Dict, output_dir: Path
    ) -> Dict[str, str]:
    ...
    def _generate_simulation_scenarios(
        self, dem_data: Dict, rainfall_data: Dict
    ) -> List[Dict]:
            ...
            scenario = {
                ...
                "config": {
                    ...
                    "output_directory": str(
                        self.output_dir / "simulations" / f"scenario_{scenario_id:04d}"
                    ),
    ...
    async def _stage_simulation_execution(self) -> Dict[str, Any]:
        ...
        batch_id = self._batch_orchestrator.create_batch_from_scenarios(
            scenarios=scenarios, output_dir=str(self.output_dir / "batch_results")
        )
    ...
    async def _stage_simulation_validation(self) -> Dict[str, Any]:
        ...
        for result in successful_results:
            if "outputs" in result and "extent_file" in result["outputs"]:
                ...
                validation_results.append(
                    {"scenario_id": result["scenario_id"], "validation": validation}
                )
        ...
        return {
            ...
            "validation_rate": (
                len(valid_simulations) / len(validation_results)
    ...
    def _create_training_manifest(self, valid_scenarios: List[Dict]) -> str:
        ...
        batch_results = self.results["simulation_execution"]["batch_results"]
        successful_results = {
            r["scenario_id"]: r for r in batch_results.get("successful_results", [])
        }
    ...
    async def _stage_final_validation(self) -> Dict[str, Any]:
        ...
        validation_report = {
            ...
            "ml_model_performance": self.results.get("ml_training_execution", {}).get(
                "training_metrics", {}
            ),
    ...
    def _generate_outputs_summary(self) -> Dict[str, Any]:
        ...
        if "simulation_execution" in self.results:
            ...
            outputs["training_manifest"] = str(
                self.output_dir / "training_manifest.json"
            )

        if (
            "ml_training_execution" in self.results
            and self.results["ml_training_execution"]["status"] != "skipped"
        ):
            outputs["ml_model"] = self.results["ml_training_execution"][
                "model_checkpoint"
            ]
            outputs["training_logs"] = self.results["ml_training_execution"][
                "training_logs"
            ]
    ...
    def _cleanup_intermediate_files(self):
        ...
        cleanup_dirs = ["simulations/*/temp", "preprocessed/intermediate", "data/temp"]
    ...
    def _generate_pipeline_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "resource_usage": (
                self.resource_manager.get_usage_summary()
