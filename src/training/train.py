...
class FloodDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        rainfall_idx = self.input_channels.index('rainfall') if 'rainfall' in self.input_channels else 0
    ...
    def _generate_synthetic_data(self, channel: str, size: Tuple[int, int] = (256, 256)) -> np.ndarray:
        ...
        if channel == 'dem':
            ...
            X, Y = np.meshgrid(x, y)
            data = 100 + 50 * np.sin(0.5 * X) * np.cos(0.5 * Y) + np.random.normal(0, 5, size)
            ...
            for i in range(kernel_size, size[0] - kernel_size):
                for j in range(kernel_size, size[1] - kernel_size):
                    smoothed[i, j] = np.mean(
                        base[i-kernel_size:i+kernel_size+1, j-kernel_size:j+kernel_size+1]
                    )
            data = 5 + 20 * smoothed
    ...
    def _generate_synthetic_target(self, size: Tuple[int, int] = (256, 256)) -> np.ndarray:
        ...
        flood_depth = np.maximum(0, rainfall_norm * 2.0 - dem_norm * 1.5 + 0.1 * np.random.random(size))
...
class FloodTrainer:
    ...
    def save_checkpoint(self, epoch: int, is_best: bool = False) -> None:
        ...
        if is_best:
            ...
            logger.info(f"New best model saved with validation loss: {self.best_val_loss:.6f}")
    ...
    def plot_training_curves(self) -> None:
        ...
        plt.tight_layout()
        plt.savefig(Path(self.config['checkpoint_dir']) / 'training_curves.png', dpi=150)
        plt.close()
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.config['epochs'] + 1):
            ...
            # Log metrics
            logger.info(f"Train Loss: {train_metrics['total']:.6f} | "
                       f"Val Loss: {val_metrics['total']:.6f} | "
                       f"LR: {current_lr:.2e}")
            logger.info(f"Train MSE: {train_metrics['mse']:.6f} | "
                       f"Val MSE: {val_metrics['mse']:.6f}")
            logger.info(f"Train Physics: {train_metrics['physics']:.6f} | "
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default='data/flood_data',
