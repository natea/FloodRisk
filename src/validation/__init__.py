...
class ValidationFramework:
    ...
    def _default_config(self):
        ...
        return {
            ...
            "validation": {"lisflood_comparison": True, "nfip_validation": True},
            "visualization": {"generate_maps": True, "interactive_plots": True},
            "reporting": {
                "generate_html": True,
                "generate_pdf": False,
    ...
    def validate_model(self, predictions, observations, metadata=None):
        ...
        if self.config["metrics"]["calculate_regression"]:
            results["regression"] = self.metrics.calculate_regression_metrics(
                predictions, observations
            )

        if self.config["metrics"]["calculate_classification"]:
            results["classification"] = self.metrics.calculate_classification_metrics(
                predictions, observations
            )

        # Generate visualizations
        if self.config["visualization"]["generate_maps"]:
            results["visualizations"] = self.viz.create_comparison_map(
