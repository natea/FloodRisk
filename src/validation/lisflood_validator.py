    ...
    def read_netcdf_file(filepath: Union[str, Path]) -> Dict:
        ...
        try:
            ...
            if depth_var is None:
                ...
                raise ValueError(f"No depth variable found. Available: {available_vars}")
...
class SpatialAligner:
    ...
    def __init__(self, tolerance: float = 1e-6, method: str = 'bilinear'):
        ...
        self.method = method
        logger.info(f"SpatialAligner initialized: tolerance={tolerance}, method={method}")
    
    def align_grids(self, model_data: np.ndarray, reference_data: Dict, 
                   model_transform=None, target_transform=None) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        try:
            ...
            if model_transform is None or ref_transform is None:
                logger.warning("No transform information - assuming grids are spatially aligned")
            ...
            return self._align_with_rasterio(model_data, ref_data, model_transform, ref_transform)
            
        except Exception as e:
            logger.error(f"Error aligning grids: {e}")
            raise MetricError(f"Grid alignment failed: {e}")
    
    def _align_by_shape(self, model_data: np.ndarray, ref_data: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _align_with_rasterio(self, model_data: np.ndarray, ref_data: np.ndarray,
                           model_transform, ref_transform) -> Tuple[np.ndarray, np.ndarray]:
        """
...
class LISFLOODValidator:
    ...
    def __init__(self, config: Optional[LISFLOODConfig] = None):
        ...
        self.spatial_aligner = SpatialAligner(
            tolerance=self.config.spatial_tolerance,
            method=self.config.resample_method
        )
        
        logger.info(f"LISFLOOD Validator initialized with reference: {self.config.reference_path}")
    
    def validate_against_lisflood(self, model_predictions: np.ndarray, 
        ...
        try:
            ...
            model_aligned, ref_aligned = self.spatial_aligner.align_grids(
                model_clean, reference_data,
                model_metadata.get('transform') if model_metadata else None
            )
            
            # Calculate validation metrics
            validation_results = self._calculate_validation_metrics(model_aligned, ref_aligned)
            
            # Spatial statistics
            if self.config.calculate_spatial_stats:
                spatial_stats = self._calculate_spatial_statistics(model_aligned, ref_aligned)
    ...
    def _load_reference_data(self, reference_file: Union[str, Path]) -> Dict:
        ...
        if ref_path.suffix.lower() == '.asc':
            ...
            logger.warning(f"Unknown file extension {ref_path.suffix}, trying ASC format")
            return self.file_reader.read_asc_file(ref_path)
    
    def _preprocess_data(self, model_data: np.ndarray, reference_data: Dict,
                        model_metadata: Optional[Dict] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _calculate_validation_metrics(self, model_data: np.ndarray, 
                                    reference_data: np.ndarray) -> Dict:
        """Calculate comprehensive validation metrics"""
        logger.info("Calculating validation metrics")
        
        # Get all standard metrics
        all_metrics = self.metrics_calc.calculate_all_metrics(model_data, reference_data)
        
        # Add LISFLOOD-specific metrics
        lisflood_metrics = self._calculate_lisflood_specific_metrics(model_data, reference_data)
        
        return {
            'standard_metrics': all_metrics,
            'lisflood_specific': lisflood_metrics
        }
    
    def _calculate_lisflood_specific_metrics(self, model_data: np.ndarray,
                                           reference_data: np.ndarray) -> Dict:
        ...
        extent_metrics = self.metrics_calc.calculate_classification_metrics(model_extent, ref_extent)
        ...
        volume_metrics = {
            ...
            'volume_bias_percent': 100 * (model_volume - ref_volume) / ref_volume if ref_volume > 0 else np.inf
        }
    ...
    def _calculate_spatial_statistics(self, model_data: np.ndarray,
                                    reference_data: np.ndarray) -> Dict:
        ...
        if np.sum(valid_mask) > 1:
            spatial_corr = np.corrcoef(model_data[valid_mask], reference_data[valid_mask])[0, 1]
        else:
            spatial_corr = np.nan
        
        # Spatial pattern analysis could be expanded here
        spatial_stats = {
            'spatial_correlation': float(spatial_corr) if not np.isnan(spatial_corr) else None,
    ...
    def generate_comparison_report(self, validation_results: Dict, output_path: str) -> None:
        """
    ...
    def _generate_recommendations(self, results: Dict) -> List[str]:
        ...
        if 'iou' in standard:
            ...
            if iou_score < 0.5:
                recommendations.append("Low IoU score suggests poor flood extent prediction - review model parameters")
            elif iou_score > 0.8:
                recommendations.append("Excellent flood extent prediction - model performs well")
        
        # RMSE recommendations
        if 'regression' in standard and 'rmse' in standard['regression']:
            rmse = standard['regression']['rmse']
            if rmse > 1.0:
                recommendations.append("High RMSE indicates significant depth prediction errors - consider model calibration")
        ...
        if 'volumetric_analysis' in lisflood:
            ...
            if abs(vol_bias) > 20:
                recommendations.append(f"Large volume bias ({vol_bias:.1f}%) - check model water balance")
        
        return recommendations


# Utility functions
def batch_validate_lisflood(model_files: List[str], reference_files: List[str],
                           config: Optional[LISFLOODConfig] = None) -> pd.DataFrame:
    """
