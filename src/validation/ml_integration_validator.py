...
class MLDataValidator(BaseValidator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.expected_input_shape = config.get(
        ...
        try:
            ...
            if hasattr(dataset, "__len__"):
                ...
                if dataset_size == 0:
                    ...
                    return self._create_result(
                        "ML_Data_Compatibility", "FAIL", score, details, issues
                    )

                # Sample a few items to check format
                sample_indices = np.random.choice(
                    min(10, dataset_size), size=min(5, dataset_size), replace=False
                )
                ...
                if not sample_validation["shape_valid"]:
                    issues.append(
                        f"Invalid tensor shapes: expected input {self.expected_input_shape}, output {self.expected_output_shape}"
                    )
                    score -= 0.3
            ...
            if hasattr(dataset, "__getitem__"):
                ...
                if memory_analysis["estimated_memory_gb"] > self.max_memory_gb:
                    issues.append(
                        f"High memory requirement ({memory_analysis['estimated_memory_gb']:.2f}GB) exceeds limit"
                    )
                    score -= 0.2
            ...
            self.logger.info(
                f"ML data validation completed: {status} (score: {score:.3f})"
            )
            return self._create_result(
                "ML_Data_Compatibility", status, max(0.0, score), details, issues
            )

        except Exception as e:
            self.logger.error(f"ML data validation failed: {e}")
            return self._create_result(
                "ML_Data_Compatibility",
                "FAIL",
                0.0,
                {"error": str(e)},
                ...
                if isinstance(sample, (tuple, list)) and len(sample) == 2:
                    ...
                    if hasattr(input_data, "shape"):
                        ...
                        if len(input_shape) == len(expected_shape):
                            ...
                            len(input_shape) == len(expected_shape) + 1
                        ):  # with batch dim
                            if input_shape[-len(expected_shape) :] != expected_shape:
                                shape_valid = False
                        else:
                    ...
                    if hasattr(target_data, "shape"):
                        ...
                        if len(target_shape) == len(expected_shape):
                            ...
                            len(target_shape) == len(expected_shape) + 1
                        ):  # with batch dim
                            if target_shape[-len(expected_shape) :] != expected_shape:
                                shape_valid = False
                        else:
        ...
        try:
            ...
            if isinstance(batch, (tuple, list)) and len(batch) == 2:
                ...
                return {
                    ...
                    "batch_input_shape": (
                        list(inputs.shape) if hasattr(inputs, "shape") else "unknown"
                    ),
                    "batch_target_shape": (
                        list(targets.shape) if hasattr(targets, "shape") else "unknown"
                    ),
                    "batch_size": (
                        len(inputs) if hasattr(inputs, "__len__") else "unknown"
                    ),
                }
            else:
                return {
            ...
            return {"batch_loading_success": False, "error": str(e)}
    ...
    def _validate_data_types(self, dataset: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(sample, (tuple, list)) and len(sample) == 2:
                ...
                input_valid = hasattr(input_data, "dtype") and hasattr(
                    input_data, "shape"
                )
                target_valid = hasattr(target_data, "dtype") and hasattr(
                    target_data, "shape"
                )
                ...
                if target_valid:
                    target_numeric = any(
                        t in str(target_data.dtype) for t in ["float", "int", "long"]
                    )

                return {
                    "dtypes_valid": input_valid
                    and target_valid
                    and input_float
                    and target_numeric,
                    "input_dtype": str(input_data.dtype) if input_valid else "unknown",
                    "target_dtype": (
                        str(target_data.dtype) if target_valid else "unknown"
                    ),
...
class ModelPerformanceValidator(BaseValidator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.inference_speed_threshold = config.get(
            "inference_speed_threshold", 100
        )  # ms per sample
    ...
    def validate(self, model_results: Dict[str, Any], **kwargs) -> ValidationResult:
        ...
        try:
            ...
            if "training_history" in model_results:
                training_analysis = self._analyze_training_history(
                ...
                if training_analysis["overfitting_detected"]:
                    issues.append(
                        f"Overfitting detected (gap: {training_analysis['train_val_gap']:.3f})"
                    )
                    score -= 0.2

            # 2. Final performance validation
            if "test_metrics" in model_results:
                performance_analysis = self._analyze_test_performance(
                ...
                if performance_analysis["accuracy"] < self.min_accuracy:
                    issues.append(
                        f"Low test accuracy ({performance_analysis['accuracy']:.3f}) below threshold"
                    )
                    score -= 0.3

            # 3. Real vs dummy data comparison
            if (
                "real_data_metrics" in model_results
                and "dummy_data_metrics" in model_results
            ):
                ...
                if comparison_analysis["real_significantly_better"]:
                    ...
                    issues.append(
                        "Dummy data performs better than real data - possible data quality issues"
                    )
                    score -= 0.3
                else:
                    issues.append(
                        "No significant performance difference between real and dummy data"
                    )
                    score -= 0.1

            # 4. Inference speed validation
            if "inference_metrics" in model_results:
                inference_analysis = self._analyze_inference_performance(
                    model_results["inference_metrics"]
                )
                details["inference_analysis"] = inference_analysis

                if (
                    inference_analysis["avg_inference_time_ms"]
                    > self.inference_speed_threshold
                ):
                    issues.append(
                        f"Slow inference speed ({inference_analysis['avg_inference_time_ms']:.1f}ms)"
                    )
                    score -= 0.1

            # 5. Model architecture validation
            if "model_info" in model_results:
                architecture_analysis = self._validate_model_architecture(
                    model_results["model_info"]
                )
                details["architecture_analysis"] = architecture_analysis

                if (
                    architecture_analysis["parameter_count"] > 100_000_000
                ):  # 100M parameters
            ...
            self.logger.info(
                f"Model performance validation completed: {status} (score: {score:.3f})"
            )
            return self._create_result(
                "Model_Performance", status, max(0.0, score), details, issues
            )
    ...
    def _analyze_training_history(
        ...
        if len(train_losses) >= self.min_convergence_epochs:
            ...
            recent_losses = (
                train_losses[-5:] if len(train_losses) >= 5 else train_losses
            )
            ...
            if val_losses:
                ...
                if len(val_losses) >= 10:
                    recent_val_trend = np.polyfit(range(5), val_losses[-5:], 1)[
                        0
                    ]  # slope
                    recent_train_trend = np.polyfit(range(5), train_losses[-5:], 1)[0]

                    analysis["overfitting_detected"] = (
                        recent_val_trend > 0 and recent_train_trend < 0
                    )
        ...
        if (
            train_accuracies
            and val_accuracies
            and len(train_accuracies) == len(val_accuracies)
        ):
    ...
    def _analyze_test_performance(
        ...
        return {
            ...
            "meets_accuracy_threshold": test_metrics.get("accuracy", 0.0)
            >= self.min_accuracy,
        }

    def _compare_real_vs_dummy(
        self, real_metrics: Dict[str, float], dummy_metrics: Dict[str, float]
    ) -> Dict[str, Any]:
    ...
    def _analyze_inference_performance(
        ...
        return {
            ...
            "meets_speed_threshold": inference_metrics.get("avg_time_ms", float("inf"))
            <= self.inference_speed_threshold,
        }
...
class LabelQualityValidator(BaseValidator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        super().__init__(config)
        self.min_class_ratio = config.get(
            "min_class_ratio", 0.01
        )  # At least 1% of minority class
        self.max_class_ratio = config.get(
            "max_class_ratio", 0.99
        )  # At most 99% of majority class
        self.spatial_coherence_threshold = config.get(
    ...
    def validate(self, labels_data: Dict[str, Any], **kwargs) -> ValidationResult:
        ...
        try:
            ...
            if class_analysis["imbalance_severe"]:
                issues.append(
                    f"Severe class imbalance detected (minority class: {class_analysis['minority_ratio']:.3f})"
                )
                score -= 0.3
            elif class_analysis["imbalance_moderate"]:
                issues.append(
                    f"Moderate class imbalance (minority class: {class_analysis['minority_ratio']:.3f})"
                )
                score -= 0.1
            ...
            if consistency_analysis["inconsistent_labels"] > 0.05:  # 5% inconsistent
                issues.append(
                    f"High label inconsistency ({consistency_analysis['inconsistent_labels']*100:.1f}%)"
                )
                score -= 0.2
            ...
            if labels.ndim >= 2:
                ...
                if (
                    spatial_analysis["coherence_score"]
                    < self.spatial_coherence_threshold
                ):
                    issues.append(
                        f"Low spatial coherence in labels ({spatial_analysis['coherence_score']:.3f})"
                    )
                    score -= 0.2
            ...
            if noise_analysis["estimated_noise_ratio"] > 0.1:  # 10% noise
                issues.append(
                    f"High estimated label noise ({noise_analysis['estimated_noise_ratio']*100:.1f}%)"
                )
                score -= 0.2
            ...
            self.logger.info(
                f"Label quality validation completed: {status} (score: {score:.3f})"
            )
            return self._create_result(
                "Label_Quality", status, max(0.0, score), details, issues
            )
    ...
    def _analyze_class_balance(self, labels: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "balance_score": (
                1.0 - abs(0.5 - minority_ratio)
                if len(unique_values) == 2
                else 1.0 - np.std(class_ratios)
            ),
        }
        ...
        if labels.ndim == 2:  # For 2D labels
            ...
            for i in range(1, h - 1):
                for j in range(1, w - 1):
                    ...
                    if (
                        np.sum(neighbors == center) == 1
                    ):  # Only center pixel has this value
                        inconsistent_count += 1

        inconsistent_ratio = (
            inconsistent_count / len(flat_labels) if len(flat_labels) > 0 else 0
        )
    ...
    def _validate_label_values(self, labels: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_non_integer_values": not all(
                v == int(v) for v in unique_values[:10]
            ),  # Check first 10
        }
...
class MLIntegrationValidator:
    ...
    def validate_ml_pipeline(
        ...
        if "model_results" in ml_data:
            try:
                perf_result = self.performance_validator.validate(
                    ml_data["model_results"]
                )
        ...
        for result in self.results:
            if result.component == "ML_Data_Compatibility" and result.score < 0.8:
                recommendations.append(
                    "Review dataset format and compatibility with ML training pipeline"
                )

            elif result.component == "Model_Performance" and result.score < 0.8:
                recommendations.append(
                    "Improve model training: check convergence, overfitting, hyperparameters"
                )

            elif result.component == "Label_Quality" and result.score < 0.8:
                recommendations.append(
                    "Review label quality: check class balance, consistency, noise"
                )

        if not recommendations:
            recommendations.append(
                "ML pipeline validation passed - ready for production deployment"
            )
