...
class BaseValidator(ABC):
    ...
    def _create_result(self, component: str, status: str, score: float, 
                      details: Dict[str, Any], issues: List[str] = None,
...
class DEMValidator(BaseValidator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.smoothness_threshold = config.get('smoothness_threshold', 100)  # max elevation change
        ...
        try:
            ...
            if len(valid_data) == 0:
                ...
                if min_elev < self.elevation_bounds[0]:
                    issues.append(f"Minimum elevation ({min_elev:.2f}m) below expected range")
                    score -= 0.2
                    
                if max_elev > self.elevation_bounds[1]:
                    issues.append(f"Maximum elevation ({max_elev:.2f}m) above expected range")
                    score -= 0.2
            ...
            if void_percentage > self.void_threshold:
                issues.append(f"High void percentage ({void_percentage*100:.2f}%) exceeds threshold")
                score -= 0.3
            ...
            if len(valid_data) > 0:
                ...
                if max_gradient > self.smoothness_threshold:
                    issues.append(f"High elevation gradients detected (max: {max_gradient:.2f}m/pixel)")
                    score -= 0.2
            ...
            return self._create_result('DEM_Quality', status, max(0.0, score), details, issues)
            
        except Exception as e:
            self.logger.error(f"DEM validation failed: {e}")
            return self._create_result(
                'DEM_Quality', 'FAIL', 0.0, 
...
class RainfallValidator(BaseValidator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.missing_data_threshold = config.get('missing_data_threshold', 0.1)  # 10% max missing
        
    def validate(self, rainfall_data: Union[str, Path, np.ndarray, xr.DataArray], **kwargs) -> ValidationResult:
        ...
        try:
            ...
            if len(valid_data) == 0:
                ...
                if min_val < 0:
                    issues.append(f"Negative rainfall values detected (min: {min_val:.3f})")
                    score -= 0.2
                
                # Check for extreme values
                if max_val > self.max_intensity:
                    issues.append(f"Extreme rainfall intensity ({max_val:.2f} mm/hr) exceeds threshold")
                    score -= 0.2
                ...
                if missing_percentage > self.missing_data_threshold:
                    issues.append(f"High missing data percentage ({missing_percentage*100:.2f}%)")
                    score -= 0.3
                ...
                if data_array.ndim >= 2:
                    ...
                    if not np.all(np.isnan(spatial_data)):
                        ...
                        details['spatial_analysis'] = {
                            ...
                            'coefficient_of_variation': spatial_std / spatial_mean if spatial_mean > 0 else np.inf
                        }
                ...
                if has_time and data_array.ndim == 3:
                    ...
                    temporal_consistency = 1.0 - (np.nanstd(temporal_means) / np.nanmean(temporal_means)) if np.nanmean(temporal_means) > 0 else 0.0
                    ...
                    if temporal_consistency < 0.5:
                        issues.append(f"Poor temporal consistency ({temporal_consistency:.3f})")
                        score -= 0.2
            ...
            self.logger.info(f"Rainfall validation completed: {status} (score: {score:.3f})")
            return self._create_result('Rainfall_Quality', status, max(0.0, score), details, issues)
            
        except Exception as e:
            self.logger.error(f"Rainfall validation failed: {e}")
            return self._create_result(
                'Rainfall_Quality', 'FAIL', 0.0,
                {'error': str(e)}, [f"Validation failed: {e}"]
            )
...
class SpatialConsistencyValidator(BaseValidator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        super().__init__(config)
        self.tolerance = config.get('spatial_tolerance', 0.1)  # tolerance in coordinate units
        ...
        try:
            ...
            for dataset in datasets:
                try:
                    ...
                    issues.append(f"Could not read spatial info from {dataset['path']}: {e}")
                    score -= 0.3
            
            if len(dataset_info) < 2:
                return self._create_result(
                    'Spatial_Consistency', 'FAIL', 0.0,
            ...
            for i, info in enumerate(dataset_info[1:], 1):
                if info['crs'] != reference_crs:
                    issues.append(f"CRS mismatch: {dataset_info[0]['name']} ({reference_crs}) vs {info['name']} ({info['crs']})")
            ...
            if overlap_analysis['min_overlap'] < self.min_overlap:
                issues.append(f"Insufficient spatial overlap ({overlap_analysis['min_overlap']:.3f}) below threshold")
                score -= 0.3
            
            details = {
                'datasets': [{'name': info['name'], 'crs': str(info['crs']), 'bounds': info['bounds'], 
                             'shape': info['shape'], 'resolution': info['resolution']} for info in dataset_info],
            ...
            if not resolution_analysis['consistent']:
                issues.append(f"Resolution inconsistency detected: {resolution_analysis['details']}")
                score -= 0.2
            ...
            self.logger.info(f"Spatial consistency validation completed: {status} (score: {score:.3f})")
            return self._create_result('Spatial_Consistency', status, max(0.0, score), details, issues)
    ...
    def _analyze_spatial_extents(self, dataset_info: List[Dict]) -> Dict[str, Any]:
        ...
        intersection_area = max(0, (min_right - max_left)) * max(0, (min_top - max_bottom))
    ...
    def _analyze_resolutions(self, dataset_info: List[Dict]) -> Dict[str, Any]:
        ...
        for i, res in enumerate(resolutions[1:], 1):
            ...
            if x_diff > self.tolerance or y_diff > self.tolerance:
                consistent = False
                details.append(f"{dataset_info[i]['name']}: {res} vs reference {reference_res}")
    ...
    def _calculate_spatial_overlap(self, dataset_info: List[Dict]) -> Dict[str, Any]:
        ...
        mean_overlap = np.mean([o['mutual_overlap'] for o in overlaps]) if overlaps else 0.0
...
class SimulationValidator(BaseValidator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.mass_conservation_tolerance = config.get('mass_conservation_tolerance', 0.05)  # 5%
        self.convergence_threshold = config.get('convergence_threshold', 1e-6)
        
    def validate(self, simulation_results: Dict[str, Any], **kwargs) -> ValidationResult:
        ...
        try:
            ...
            if depths is None:
                ...
                if len(valid_depths) > 0:
                    depth_stats = {
                        ...
                        'flooded_fraction': float(np.sum(valid_depths > 0.01) / len(valid_depths))
                    }
                    details['depth_analysis'] = depth_stats
                    
                    # Check for negative depths
                    if depth_stats['min'] < 0:
                        issues.append(f"Negative flood depths detected (min: {depth_stats['min']:.3f}m)")
                        score -= 0.3
                    
                    # Check for extreme depths
                    if depth_stats['max'] > self.max_depth:
                        issues.append(f"Extreme flood depth ({depth_stats['max']:.2f}m) exceeds threshold")
                        score -= 0.2
                    ...
                    if depths.ndim >= 2:
                        ...
                        if gradient_check['extreme_gradients'] > 0.01:  # 1% of pixels
                            issues.append(f"High number of extreme gradients ({gradient_check['extreme_gradients']*100:.2f}%)")
                            score -= 0.2
                ...
                if 'inflow' in simulation_results and 'outflow' in simulation_results:
                    ...
                    if mass_balance['conservation_error'] > self.mass_conservation_tolerance:
                        issues.append(f"Poor mass conservation (error: {mass_balance['conservation_error']*100:.2f}%)")
                        score -= 0.3
                ...
                if convergence_info:
                    ...
                    if not convergence_analysis['converged']:
                        ...
                        score -= 0.4
                    elif convergence_analysis['iterations'] > convergence_analysis.get('max_iterations', 1000) * 0.9:
                        issues.append("Simulation required excessive iterations to converge")
                        score -= 0.1
                ...
                if velocities is not None:
                    ...
                    if velocity_analysis['max_velocity'] > 20.0:  # 20 m/s threshold
                        issues.append(f"Extreme velocities detected (max: {velocity_analysis['max_velocity']:.2f} m/s)")
                        score -= 0.2
            ...
            self.logger.info(f"Simulation validation completed: {status} (score: {score:.3f})")
            return self._create_result('Simulation_Quality', status, max(0.0, score), details, issues)
            
        except Exception as e:
            self.logger.error(f"Simulation validation failed: {e}")
            return self._create_result(
                'Simulation_Quality', 'FAIL', 0.0,
                {'error': str(e)}, [f"Validation failed: {e}"]
            )
    ...
    def _check_spatial_gradients(self, depths: np.ndarray) -> Dict[str, Any]:
        ...
        extreme_gradients = np.sum(valid_gradients > extreme_threshold) / len(valid_gradients)
    ...
    def _check_mass_conservation(self, inflow: float, outflow: float, depths: np.ndarray) -> Dict[str, Any]:
        """Check mass conservation in simulation"""
        # Calculate stored volume
        valid_depths = depths[~np.isnan(depths) & (depths > 0)]
        stored_volume = float(np.sum(valid_depths))  # Simplified, assumes unit cell area
        ...
        return {
            ...
            'balance_check': 'PASS' if conservation_error < self.mass_conservation_tolerance else 'FAIL'
        }
    ...
    def _validate_velocities(self, velocities: np.ndarray, depths: np.ndarray) -> Dict[str, Any]:
        ...
        valid_velocities = velocity_magnitude[water_mask & ~np.isnan(velocity_magnitude)]
        ...
        return {
            ...
            'high_velocity_fraction': float(np.sum(valid_velocities > 5.0) / len(valid_velocities))
        }
...
class TileQualityValidator(BaseValidator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        super().__init__(config)
        self.target_flood_ratio = config.get('target_flood_ratio', (0.1, 0.9))  # 10-90% flood coverage
    ...
    def validate(self, tiles_info: Dict[str, Any], **kwargs) -> ValidationResult:
        ...
        try:
            ...
            if num_tiles < self.min_tiles:
                issues.append(f"Insufficient tiles ({num_tiles}) below minimum threshold ({self.min_tiles})")
                score -= 0.2
            ...
            for i, tile in enumerate(tiles_data):
                ...
                if tile_data is not None:
                    ...
                    edge_sum = (np.sum(tile_data[:self.edge_threshold, :]) + 
                               np.sum(tile_data[-self.edge_threshold:, :]) +
                               np.sum(tile_data[:, :self.edge_threshold]) +
                               np.sum(tile_data[:, -self.edge_threshold:]))
                    
                    center_area = tile_data[self.edge_threshold:-self.edge_threshold, 
            ...
            if flood_ratios:
                ...
                balance_analysis = {
                    ...
                    'tiles_in_range': int(np.sum((flood_ratios >= self.target_flood_ratio[0]) & 
                ...
                tiles_in_range_ratio = balance_analysis['tiles_in_range'] / len(flood_ratios)
                if tiles_in_range_ratio < 0.7:  # Less than 70% in good range
                    issues.append(f"Poor flood/dry balance: only {tiles_in_range_ratio*100:.1f}% of tiles in target range")
                    score -= 0.3
                
                # Check for too many extreme tiles
                extreme_tiles = balance_analysis['all_dry_tiles'] + balance_analysis['all_flooded_tiles']
                if extreme_tiles / len(flood_ratios) > 0.3:  # More than 30% extreme
                    issues.append(f"Too many extreme tiles ({extreme_tiles} all-dry or all-flooded)")
                    score -= 0.2
            ...
            if edge_issues > len(tiles_data) * 0.1:  # More than 10% with edge issues
                issues.append(f"High number of tiles with potential edge artifacts ({edge_issues})")
                score -= 0.2
            
            # 5. Metadata validation
            if tile_metadata:
                details['metadata_validation'] = self._validate_tile_metadata(tile_metadata)
            ...
            self.logger.info(f"Tile quality validation completed: {status} (score: {score:.3f})")
            return self._create_result('Tile_Quality', status, max(0.0, score), details, issues)
    ...
    def _validate_tile_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'tile_size' in metadata:
            ...
            validation_results['tile_size_valid'] = isinstance(tile_size, (int, tuple, list))
        
        if 'overlap' in metadata:
            overlap = metadata['overlap']
            validation_results['overlap_valid'] = isinstance(overlap, (int, float)) and 0 <= overlap < 1
...
class PipelineValidator:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.spatial_validator = SpatialConsistencyValidator(self.config.get('spatial', {}))
        self.simulation_validator = SimulationValidator(self.config.get('simulation', {}))
        self.tile_validator = TileQualityValidator(self.config.get('tiles', {}))
        
        self.results = []
    
    def validate_full_pipeline(self, pipeline_data: Dict[str, Any]) -> Dict[str, ValidationResult]:
        """
        ...
        if 'rainfall_data' in pipeline_data:
            try:
                rainfall_result = self.rainfall_validator.validate(pipeline_data['rainfall_data'])
        ...
        if 'spatial_datasets' in pipeline_data:
            try:
                spatial_result = self.spatial_validator.validate(pipeline_data['spatial_datasets'])
                validation_results['spatial_consistency'] = spatial_result
                self.results.append(spatial_result)
                self.logger.info(f"Spatial consistency validation: {spatial_result.status}")
            except Exception as e:
                self.logger.error(f"Spatial consistency validation failed: {e}")
        ...
        if 'simulation_results' in pipeline_data:
            try:
                sim_result = self.simulation_validator.validate(pipeline_data['simulation_results'])
    ...
    def generate_validation_report(self, output_path: Union[str, Path] = None) -> Dict[str, Any]:
        """
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        for result in self.results:
            if result.status == 'FAIL':
                recommendations.append(f"CRITICAL: Address issues in {result.component} before proceeding")
            elif result.status == 'WARN':
                recommendations.append(f"WARNING: Review and potentially fix issues in {result.component}")
            
            # Component-specific recommendations
            if result.component == 'DEM_Quality' and result.score < 0.8:
                recommendations.append("Consider DEM preprocessing: void filling, smoothing, or elevation correction")
            
            elif result.component == 'Rainfall_Quality' and result.score < 0.8:
                recommendations.append("Review rainfall data quality: check for missing values, extreme outliers")
            
            elif result.component == 'Spatial_Consistency' and result.score < 0.8:
                recommendations.append("Ensure all datasets use consistent CRS and spatial extents")
            
            elif result.component == 'Simulation_Quality' and result.score < 0.8:
                recommendations.append("Check simulation convergence and physical plausibility of results")
            
            elif result.component == 'Tile_Quality' and result.score < 0.8:
                recommendations.append("Review tile generation: balance flood/dry ratios, check for edge effects")
        
        if not recommendations:
            recommendations.append("All validation checks passed successfully - pipeline ready for ML training")
