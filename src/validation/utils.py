...
class DataLoader:
    ...
    def load_flood_data(self, file_path: Union[str, Path], 
...
class SpatialUtils:
    ...
    @staticmethod
    def transform_coordinates(coords: np.ndarray, src_crs: str, dst_crs: str) -> np.ndarray:
        """
    ...
    @staticmethod
    def align_grids(grid1: np.ndarray, grid2: np.ndarray, 
                   method: str = 'crop') -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        if method == 'crop':
            ...
            aligned_grid1 = np.pad(grid1, 
                                 ((0, max_rows - grid1.shape[0]), 
                                  (0, max_cols - grid1.shape[1])), 
                                 constant_values=np.nan)
            aligned_grid2 = np.pad(grid2,
                                 ((0, max_rows - grid2.shape[0]),
                                  (0, max_cols - grid2.shape[1])),
                                 constant_values=np.nan)
...
class StatisticalUtils:
    ...
    @staticmethod
    def bootstrap_confidence_interval(data1: np.ndarray, data2: np.ndarray,
...
class LoggingSetup:
    ...
    def setup_logging(config_manager: ConfigurationManager) -> None:
        ...
        if log_config.get('console_logging', {}).get('enabled', True):
            ...
            console_level = getattr(logging, log_config.get('console_logging', {}).get('level', 'INFO').upper())
...
def log_execution_time(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.info(f"Completed {func.__name__} in {execution_time.total_seconds():.2f}s")
            return result
            
        except Exception as e:
            execution_time = datetime.now() - start_time
            logger.error(f"Failed {func.__name__} after {execution_time.total_seconds():.2f}s: {e}")
            raise
...
class PerformanceTimer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            logger.info(f"Completed {self.operation_name} in {duration.total_seconds():.2f}s")
        else:
            logger.error(f"Failed {self.operation_name} after {duration.total_seconds():.2f}s")
