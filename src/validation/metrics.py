...
class BaseMetric(ABC):
    ...
    @abstractmethod
    def calculate(self, predictions: np.ndarray, observations: np.ndarray) -> Union[float, Dict]:
        """Calculate the metric"""
        pass
    
    def _validate_inputs(self, predictions: np.ndarray, observations: np.ndarray) -> None:
        """Validate input arrays"""
        if predictions.shape != observations.shape:
            raise MetricError(f"Shape mismatch: predictions {predictions.shape} vs observations {observations.shape}")
...
class IoUCalculator(BaseMetric):
    ...
    def calculate(self, predictions: np.ndarray, observations: np.ndarray) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if union_area == 0:
                ...
                logger.warning("Union area is zero - no flood extent in either prediction or observation")
            else:
...
class RegressionMetrics(BaseMetric):
    ...
    def calculate(self, predictions: np.ndarray, observations: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"Regression metrics calculated: MAE={mae:.4f}, RMSE={rmse:.4f}, R²={r_squared:.4f}")
            return result
    ...
    def _calculate_mae(self, predictions: np.ndarray, observations: np.ndarray) -> float:
        """Mean Absolute Error"""
        return float(np.mean(np.abs(predictions - observations)))
    
    def _calculate_rmse(self, predictions: np.ndarray, observations: np.ndarray) -> float:
        """Root Mean Square Error"""
        return float(np.sqrt(np.mean((predictions - observations) ** 2)))
    
    def _calculate_bias(self, predictions: np.ndarray, observations: np.ndarray) -> float:
        """Mean Bias"""
        return float(np.mean(predictions - observations))
    
    def _calculate_r_squared(self, predictions: np.ndarray, observations: np.ndarray) -> float:
    ...
    def _calculate_nse(self, predictions: np.ndarray, observations: np.ndarray) -> float:
    ...
    def _calculate_mape(self, predictions: np.ndarray, observations: np.ndarray) -> float:
        ...
        return float(np.mean(np.abs((observations[mask] - predictions[mask]) / observations[mask]) * 100))
    
    def _calculate_pbias(self, predictions: np.ndarray, observations: np.ndarray) -> float:
        """Percent Bias"""
    ...
    def _calculate_kge(self, predictions: np.ndarray, observations: np.ndarray) -> float:
...
class ClassificationMetrics(BaseMetric):
    ...
    def calculate(self, predictions: np.ndarray, observations: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            mcc = self._calculate_mcc(tp, fp, tn, fn)  # Matthews Correlation Coefficient
            ...
            logger.info(f"Classification metrics calculated: F1={f1:.4f}, Accuracy={accuracy:.4f}")
            return result
    ...
    def _calculate_confusion_matrix(self, predictions: np.ndarray, observations: np.ndarray) -> Tuple[int, int, int, int]:
    ...
    def calculate(self, predictions: np.ndarray, observations: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            if denominator == 0:
                ...
                logger.warning("No positive predictions or observations for CSI calculation")
            else:
                csi = tp / denominator
...
class MetricsCalculator:
    ...
    def calculate_all_metrics(self, predictions: np.ndarray, observations: np.ndarray) -> Dict[str, Dict]:
        """
        ...
        try:
            ...
            try:
                results['regression'] = self.regression_calc.calculate(predictions, observations)
            ...
            try:
                results['classification'] = self.classification_calc.calculate(predictions, observations)
            except MetricError as e:
    ...
    def calculate_iou(self, predictions: np.ndarray, observations: np.ndarray) -> Dict[str, float]:
        """Calculate IoU metrics only"""
        return self.iou_calc.calculate(predictions, observations)
    
    def calculate_regression_metrics(self, predictions: np.ndarray, observations: np.ndarray) -> Dict[str, float]:
        """Calculate regression metrics only"""
        return self.regression_calc.calculate(predictions, observations)
    
    def calculate_classification_metrics(self, predictions: np.ndarray, observations: np.ndarray) -> Dict[str, float]:
        """Calculate classification metrics only"""
        return self.classification_calc.calculate(predictions, observations)
    
    def calculate_csi(self, predictions: np.ndarray, observations: np.ndarray) -> Dict[str, float]:
...
def preprocess_flood_data(predictions: np.ndarray, observations: np.ndarray, 
                         min_depth: float = 0.01, max_depth: float = None) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    """
    logger.info(f"Preprocessing flood data with min_depth={min_depth}, max_depth={max_depth}")
...
def calculate_flood_extent_metrics(depth_predictions: np.ndarray, depth_observations: np.ndarray,
                                 depth_threshold: float = 0.01) -> Dict[str, float]:
    """
    ...
    extent_metrics = {
        **iou_results,
        **{k: v for k, v in class_results.items() if k in ['f1_score', 'precision', 'recall', 'accuracy']},
