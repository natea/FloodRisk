...
class NFIPDataProcessor:
    ...
    def _process_claims_data(self, claims_df: pd.DataFrame) -> pd.DataFrame:
        ...
        claims_df["amount_paid"] = pd.to_numeric(
            claims_df["amount_paid"], errors="coerce"
        )
        ...
        claims_df = claims_df[
            (claims_df["longitude"] >= -180)
            & (claims_df["longitude"] <= 180)
            & (claims_df["latitude"] >= -90)
            & (claims_df["latitude"] <= 90)
        ]
        ...
        if self.config.exclude_wind_damage and "cause_of_damage" in claims_df.columns:
            ...
            wind_mask = (
                claims_df["cause_of_damage"]
                .str.lower()
                .str.contains("|".join(wind_keywords), na=False)
            )
        ...
        logger.info(
            f"Processed claims data: {len(claims_df)} valid records (removed {initial_count - len(claims_df)})"
        )
...
class SpatialMatcher:
    ...
    def match_claims_to_predictions(
        self,
        ...
        try:
            ...
            geometry = [
                Point(row.longitude, row.latitude) for _, row in claims_df.iterrows()
            ]
            ...
            if event_date:
                ...
                logger.info(
                    f"Filtered to {len(claims_gdf)} claims within {time_window.days} days of event"
                )

            # Spatial join
            matched_data = gpd.sjoin(
                claims_gdf, pred_gdf, how="inner", predicate="within"
            )
    ...
    def _simple_grid_match(
        self, claims_df: pd.DataFrame, prediction_grid: Dict
    ) -> pd.DataFrame:
        """
    ...
    def _create_prediction_polygons(self, prediction_grid: Dict) -> gpd.GeoDataFrame:
        ...
        for i in range(10):  # Example grid
            for j in range(10):
                ...
                predictions.append(
                    prediction_grid.get("data", np.zeros((10, 10)))[i, j]
                )
                grid_ids.append(f"{i}_{j}")

        pred_gdf = gpd.GeoDataFrame(
            {"grid_id": grid_ids, "predicted_depth": predictions},
            geometry=polygons,
            crs="EPSG:4326",
        )
...
class NFIPValidator:
    ...
    def validate_against_nfip(
        ...
        try:
            ...
            prediction_grid = {"data": model_predictions, **prediction_metadata}
    ...
    def _calculate_nfip_metrics(self, matched_data: pd.DataFrame) -> Dict:
        ...
        if "predicted_depth" in matched_data.columns:
            ...
            binary_predictions = (
                matched_data["predicted_depth"] > depth_threshold
            ).astype(int)
        ...
        if (
            "predicted_depth" in matched_data.columns
            and "amount_paid" in matched_data.columns
        ):
            correlation = matched_data["predicted_depth"].corr(
                matched_data["amount_paid"]
            )
            results["depth_damage_correlation"] = (
    ...
    def _create_data_summary(self, matched_data: pd.DataFrame) -> Dict:
        ...
        return {
            ...
            "damage_categories": matched_data["damage_category"]
            .value_counts()
            .to_dict(),
        }
    ...
    def _generate_nfip_recommendations(self, results: Dict) -> List[str]:
        ...
        if matched_claims == 0:
            recommendations.append(
                "No claims were matched - check spatial alignment and temporal windows"
            )
        elif matched_claims / total_claims < 0.1:
            recommendations.append(
                "Low claim matching rate - consider expanding spatial buffer or checking coordinate systems"
            )
        ...
        if "binary_prediction" in claim_val:
            ...
            if accuracy < 0.7:
                recommendations.append(
                    "Model accuracy below 70% - consider model recalibration or additional training data"
                )
            elif accuracy > 0.9:
                recommendations.append(
                    "Excellent model performance - suitable for operational use"
                )

        # Economic recommendations
        economic_val = results.get("economic_validation", {})
        if "damage_category_distribution" in economic_val:
            severe_damage = economic_val["damage_category_distribution"].get(
                "severe", 0
            )
            if severe_damage > 0.2:
                recommendations.append(
                    "High proportion of severe damage claims - focus on extreme event modeling"
                )
    ...
    def _assess_data_quality(self, results: Dict) -> Dict:
        ...
        """
        quality_assessment = {
            "matching_success_rate": results["metadata"]["matched_claims"]
            / max(results["metadata"]["total_claims"], 1),
            "temporal_coverage": (
                "good" if results["metadata"]["matched_claims"] > 10 else "limited"
            ),
    ...
    sample_data = pd.DataFrame(
        {
            ...
            "cause_of_damage": np.random.choice(
                ["flood", "storm surge", "heavy rain"], n_claims
            ),
            "property_type": np.random.choice(
                ["single_family", "condo", "mobile_home"], n_claims
            ),
        }
    )
...
def create_nfip_validation_config(claims_path: str, **kwargs) -> NFIPConfig:
    ...
    Returns:
        NFIPConfig object
    """
    return NFIPConfig(claims_data_path=claims_path, **kwargs)
