...
class ValidationDatabase:
    ...
    def store_validation_run(self, pipeline_type: str, overall_status: str, 
                           overall_score: float, results: List[ValidationResult],
    ...
    def get_component_trends(self, component: str, days: int = 30) -> pd.DataFrame:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, conn, params=(component, cutoff_date))
    ...
    def store_alert(self, severity: str, component: str, message: str, 
...
class QAVisualizer:
    ...
    def create_validation_summary_plot(self, results: List[ValidationResult], 
        ...
        # Score bar chart
        colors = ['green' if s == 'PASS' else 'orange' if s == 'WARN' else 'red' for s in statuses]
    ...
    def create_trend_analysis_plot(self, history_df: pd.DataFrame, 
                                 save_path: Optional[Path] = None) -> str:
        ...
        if history_df.empty:
            ...
            ax1.plot(history_df['timestamp'], history_df['overall_score'], 
            ...
            status_pivot = history_df.groupby(['timestamp', 'overall_status']).size().unstack(fill_value=0)
            if not status_pivot.empty:
                ax2.stackplot(status_pivot.index, 
    ...
    def create_component_heatmap(self, component_scores: Dict[str, List[float]], 
                               save_path: Optional[Path] = None) -> str:
        ...
        for component, scores in component_scores.items():
            for i, score in enumerate(scores):
                df_data.append({'Component': component, 'Run': f'Run {i+1}', 'Score': score})
        ...
        if not df_data:
            ...
            fig, ax = plt.subplots(figsize=(max(8, len(pivot_df.columns)), max(6, len(pivot_df.index))))
            sns.heatmap(pivot_df, annot=True, cmap='RdYlGn', vmin=0, vmax=1, 
                       ax=ax, fmt='.3f', cbar_kws={'label': 'Validation Score'})
            ax.set_title('Component Performance Heatmap')
    ...
    def create_interactive_dashboard(self, results: List[ValidationResult],
                                   history_df: pd.DataFrame) -> str:
        """Create interactive Plotly dashboard"""
        # Create subplot structure
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Validation Scores', 'Score Trends', 'Status Distribution', 'Component Details'),
        ...
        if results:
            ...
            scores = [r.score for r in results]
            colors = ['green' if r.status == 'PASS' else 'orange' if r.status == 'WARN' else 'red' for r in results]
            
            fig.add_trace(
                go.Bar(x=components, y=scores, marker_color=colors, name='Scores',
        ...
        if not history_df.empty:
            fig.add_trace(
                go.Scatter(x=history_df['timestamp'], y=history_df['overall_score'],
                          mode='lines+markers', name='Score Trend', line=dict(color='blue')),
                row=1, col=2
            )
        ...
        if results:
            ...
            fig.add_trace(
                go.Bar(x=components, y=issue_counts, marker_color='red', 
                      name='Issues', opacity=0.6),
                row=2, col=2
            )
        ...
        fig.update_layout(
            height=800,
            showlegend=False,
            title_text="Validation Dashboard",
            title_x=0.5
        )
...
class QAReportGenerator:
    ...
    def generate_html_report(self, results: List[ValidationResult],
        ...
        interactive_dashboard = self.visualizer.create_interactive_dashboard(results, history_df)
    ...
    def _generate_recommendations_html(self, results: List[ValidationResult]) -> str:
        ...
        for result in results:
            if result.status == 'FAIL':
                recommendations.append(f"🚨 CRITICAL: Address issues in {result.component.replace('_', ' ')} before proceeding")
            elif result.status == 'WARN':
                recommendations.append(f"⚠️ WARNING: Review and potentially fix issues in {result.component.replace('_', ' ')}")
        
        if not recommendations:
            recommendations.append("✅ All validation checks passed successfully - pipeline ready for production")
    ...
    def generate_json_report(self, results: List[ValidationResult],
    ...
    def _generate_recommendations_list(self, results: List[ValidationResult]) -> List[str]:
        """Generate recommendations as list"""
        recommendations = []
        
        for result in results:
            if result.status == 'FAIL':
                recommendations.append(f"CRITICAL: Address issues in {result.component}")
            elif result.status == 'WARN':
...
class QAAlertSystem:
    ...
    def check_and_generate_alerts(self, results: List[ValidationResult], 
                                run_id: Optional[int] = None):
        ...
        for result in results:
            ...
            if result.score < self.critical_score_threshold:
                ...
                self.logger.critical(f'CRITICAL ALERT: {result.component} score {result.score:.3f}')
            ...
            # Warning score alerts
                ...
                self.logger.warning(f'WARNING ALERT: {result.component} score {result.score:.3f}')
    ...
    def _check_trend_alerts(self, run_id: Optional[int] = None):
        ...
        if len(history) >= 3:
            ...
            if len(recent_scores) >= 3:
                ...
                declining = all(recent_scores[i] < recent_scores[i-1] for i in range(1, len(recent_scores)))
                ...
                if declining:
                    ...
                    if abs(total_decline) > self.trend_degradation_threshold:
                        ...
                        self.logger.warning('TREND ALERT: Validation scores consistently declining')
    ...
    def get_alert_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'latest_alert': alerts_df.iloc[0].to_dict() if not alerts_df.empty else None
        }
...
class QADashboard:
    ...
    def process_validation_results(self, results: List[ValidationResult],
                                 pipeline_type: str = "FloodRisk ML Pipeline",
    ...
    def get_dashboard_data(self, days: int = 30) -> Dict[str, Any]:
        ...
        return {
            'validation_history': history.to_dict('records') if not history.empty else [],
            'active_alerts': alerts.to_dict('records') if not alerts.empty else [],
            'summary_stats': {
                'total_runs': len(history),
                'avg_score': history['overall_score'].mean() if not history.empty else 0,
                'success_rate': len(history[history['overall_status'] == 'PASS']) / len(history) * 100 if not history.empty else 0,
