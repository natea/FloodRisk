...
class FloodVisualization:
    ...
    def create_comparison_map(
        ...
        try:
            ...
            if not HAS_FOLIUM:
                ...
                return self._create_static_comparison(
                    predictions, observations, metadata
                )

            # Get bounds from metadata or use defaults
            bounds = (
                metadata.get("bounds", [-95, 25, -85, 35])
                if metadata
                else [-95, 25, -85, 35]
            )
            ...
            pred_overlay = self._create_flood_overlay(
                predictions, bounds, "Predictions", "blue"
            )
            obs_overlay = self._create_flood_overlay(
                observations, bounds, "Observations", "red"
            )
    ...
    def _create_flood_overlay(
        self, data: np.ndarray, bounds: List[float], name: str, color: str
    ) -> folium.FeatureGroup:
        """
    ...
    def _create_difference_overlay(
        self, difference: np.ndarray, bounds: List[float]
    ) -> folium.FeatureGroup:
        """
    ...
    def create_performance_dashboard(
        self, validation_results: Dict, output_path: Optional[str] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            if (
                ...
                confusion_matrix = [
                    [
                        class_metrics.get("true_negatives", 0),
                        class_metrics.get("false_positives", 0),
                    ],
                    [
                        class_metrics.get("false_negatives", 0),
                        class_metrics.get("true_positives", 0),
                    ],
                ]
    ...
    def create_spatial_analysis_plots(
        self,
        predictions: np.ndarray,
        observations: np.ndarray,
        metadata: Optional[Dict] = None,
    ...
    def create_time_series_validation(
        self, temporal_data: Dict, output_path: Optional[str] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            if "prediction_std" in temporal_data:
                pred_upper = np.array(predicted_series) + np.array(
                    temporal_data["prediction_std"]
                )
                pred_lower = np.array(predicted_series) - np.array(
                    temporal_data["prediction_std"]
                )
            ...
            return {
                "figure": fig,
                "html": fig.to_html(),
                "statistics": self._calculate_time_series_stats(
                    predicted_series, observed_series
                ),
            }
    ...
    def _create_static_comparison(
        ...
        return {"static_comparison": {"image_b64": static_plot_b64, "format": "png"}}
    ...
    def _create_simple_dashboard(self, validation_results: Dict) -> Dict:
        ...
        return {"text_dashboard": dashboard_text, "format": "text"}
    ...
    def _extract_dashboard_components(self, validation_results: Dict) -> Dict:
        ...
        if (
            ...
            components["kpis"]["accuracy"] = standard_metrics["classification"].get(
                "accuracy", 0
            )
            components["kpis"]["f1_score"] = standard_metrics["classification"].get(
                "f1_score", 0
            )
        if (
            "regression" in standard_metrics
    ...
    def _calculate_time_series_stats(
        self, predictions: List[float], observations: List[float]
    ) -> Dict:
        """
        ...
        # Basic statistics
        correlation = (
            np.corrcoef(pred_array, obs_array)[0, 1] if len(pred_array) > 1 else 0
        )
