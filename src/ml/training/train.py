...
class FloodDataModule(pl.LightningDataModule):
    ...
    def __init__(self, config: DictConfig):
        ...
        self.dem_processor = DEMProcessor(
            target_crs=config.data.target_crs, resolution=config.data.resolution
        )
        self.rainfall_processor = RainfallProcessor()
        self.tile_generator = TileGenerator(
            tile_size=config.data.tile_size, overlap=config.data.tile_overlap
        )
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            ...
            logger.warning(
                "Using dummy data for demonstration. Replace with real data loading."
            )
    ...
    def _create_dummy_data(self):
        ...
        for i in range(n_train):
            self.train_tiles.append(
                {
                    "data": np.random.randn(n_channels, tile_size, tile_size).astype(
                        np.float32
                    ),
                    "labels": np.random.randint(0, 2, (1, tile_size, tile_size)).astype(
                        np.float32
                    ),
                    "tile_id": f"train_{i}",
                }
    ...
    def train_dataloader(self):
        ...
        dataset = FloodDataset(
            self.train_tiles, augmentations=self.augmentations, is_training=True
        )
...
class FloodLightningModule(pl.LightningModule):
    ...
    def __init__(self, config: DictConfig):
        ...
        self.metrics = FloodMetrics(
            threshold=config.metrics.threshold, device=self.device
        )
        ...
        self._current_epoch = (
            0  # Use private variable to avoid conflict with Lightning's property
        )

        logger.info(
            f"Initialized FloodLightningModule with {sum(p.numel() for p in self.model.parameters()):,} parameters"
        )
    ...
    def configure_optimizers(self):
        ...
        if hasattr(
            self.model, "model"
        ):  # For wrapped models (segmentation_models_pytorch)
        ...
        if self.config.training.scheduler == "onecycle":
            scheduler = torch.optim.lr_scheduler.OneCycleLR(
                optimizer,
                max_lr=[
                    self.config.training.decoder_lr,
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=self.config.training.total_epochs
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config.training.gradient_clip_val > 0:
            torch.nn.utils.clip_grad_norm_(
                self.parameters(), self.config.training.gradient_clip_val
            )
...
def train_model(config: DictConfig):
    ...
    if config.logging.get("tensorboard", {}).get("enabled", False):
        tb_logger = TensorBoardLogger(
            save_dir=config.logging.save_dir, name="tensorboard_logs"
        )
