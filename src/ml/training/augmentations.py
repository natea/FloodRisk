...
class HydrologySafeAugmentations:
    ...
    def __init__(
        ...
        self.geometric_transforms = A.Compose([
            ...
            A.RandomCrop(
                height=int(tile_size * 0.95),
                width=int(tile_size * 0.95), 
                p=0.2
            ),
        ...
        logger.info(f"Initialized hydrology-safe augmentations with {rainfall_variability*100:.1f}% rainfall variability")
    
    def apply_rainfall_randomization(
        self, 
        rainfall_channel: np.ndarray, 
        dem_channel: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def __call__(
        ...
        rainfall_augmented = self.apply_rainfall_randomization(rainfall_channel, dem_channel)
    ...
    def create_training_transforms(self) -> A.Compose:
        ...
        return A.Compose([
            ...
            A.Normalize(mean=0.0, std=1.0, p=1.0),  # Will be overridden by custom normalization
            ToTensorV2(p=1.0)
...
class MultiReturnPeriodAugmentation:
    ...
    def __init__(
        self,
        return_periods: Dict[str, float] = None,
        ...
        logger.info(f"Initialized multi-return period augmentation with periods: {periods}")
    ...
    def generate_rainfall_scenarios(
        ...
        for _ in range(n_scenarios):
            ...
            scenario['rainfall'] = np.full(rainfall_shape, rainfall_depth, dtype=np.float32)
