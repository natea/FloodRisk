...
class GeoTIFFInferencePipeline:
    ...
    def predict_large_raster(
        ...
        for i in range(0, len(tiles), self.batch_size):
            ...
            for j, (probs, pos) in enumerate(zip(batch_probs, batch_positions)):
                ...
                if mc_samples and batch_uncertainty is not None:
                    ...
                    uncertainty_output[row_start:row_end, col_start:col_end] += uncertainty_np * weight
        ...
        prob_output = np.divide(prob_output, weight_output, 
                              out=np.zeros_like(prob_output), 
                              where=weight_output != 0)
        
        if mc_samples:
            uncertainty_output = np.divide(uncertainty_output, weight_output,
        ...
        if mc_samples:
            ...
            self._save_probability_geotiff(uncertainty_output, input_data, uncertainty_path)
