...
class FloodUNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info(f"Initializing FloodUNet: {encoder_name}, {in_channels} channels, FPN={use_fpn}")
        ...
        if hasattr(encoder, 'conv1'):
            ...
            with torch.no_grad():
                if self.in_channels < 3:
                    ...
                    new_conv.weight[:, :self.in_channels] = old_conv.weight[:, :self.in_channels]
                else:
                    # More channels: replicate existing weights
                    new_conv.weight[:, :3] = old_conv.weight
                    for i in range(3, self.in_channels):
                        new_conv.weight[:, i:i+1] = old_conv.weight[:, 0:1]  # Replicate R channel
