...
class DEMProcessor:
    ...
    def load_and_reproject(self, dem_path: Path) -> xr.DataArray:
        ...
        logger.info(
            f"DEM reprojected to {self.target_crs} at {self.resolution}m resolution"
        )
        return dem_da
    ...
    def compute_derived_features(self, dem: xr.DataArray) -> Dict[str, xr.DataArray]:
        ...
        # Plan curvature
        plan_curvature = (gxx * gy**2 - 2 * gxy * gx * gy + gyy * gx**2) / (
            gx**2 + gy**2 + 1e-8
        ) ** 1.5
        features["plan_curvature"] = dem.copy(data=plan_curvature)

        # Profile curvature
        profile_curvature = (gxx * gx**2 + 2 * gxy * gx * gy + gyy * gy**2) / (
            gx**2 + gy**2 + 1e-8
        ) ** 1.5
        ...
        hand = dem_array - ndimage.minimum_filter(
            dem_array, size=int(distance_to_stream.max() / self.resolution)
        )
...
class RainfallProcessor:
    ...
    def create_uniform_rainfall_raster(
        ...
        rainfall = np.full_like(
            template_raster.values, rainfall_depth_mm, dtype=np.float32
        )
...
class TileGenerator:
    ...
    def generate_tiles(
        ...
        """
        logger.info(
            f"Generating {self.tile_size}x{self.tile_size} tiles with {self.overlap}px overlap"
        )
    ...
    def balanced_sampling(
        ...
        logger.info(
            f"Performing balanced sampling with {flood_threshold*100:.1f}% flood threshold"
        )
        ...
        logger.info(
            f"Found {len(flooded_tiles)} flooded tiles, {len(dry_tiles)} dry tiles"
        )
        ...
        logger.info(
            f"Balanced sampling: {len(flooded_tiles)} flooded + {len(dry_tiles)} dry = {len(balanced_tiles)} total"
        )
...
def normalize_features(
    features: Dict[str, np.ndarray], method: str = "per_tile"
) -> Dict[str, np.ndarray]:
    """
    ...
    for name, data in features.items():
        if method == "per_tile":
            ...
            normalized[name] = scaler.fit_transform(data.reshape(-1, 1)).reshape(
                data.shape
            )
        else:
