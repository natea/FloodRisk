...
class OptimizedFloodDataset(Dataset):
    ...
    def __init__(
        ...
        self.memory_monitor = MemoryMonitor(
            max_gb=performance_config.max_memory_usage_gb
        )
    ...
    def _load_from_memory_map(
        self, path: Path, idx: int
        ...
        if path.suffix == ".h5":
            ...
            labels = (
                mmap_data["labels"][idx]
                if "labels" in mmap_data
                else np.zeros((1, 512, 512))
            )
        else:
            data = mmap_data["data"][idx]
            labels = (
...
class DataLoadingProfiler:
    ...
    def profile_batch_loading(self, dataloader: DataLoader, num_batches: int = 10):
        ...
        logger.info(
            f"Throughput: {np.mean(self.batch_sizes) / avg_load_time:.1f} samples/sec"
        )
