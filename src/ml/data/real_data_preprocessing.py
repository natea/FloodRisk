...
try:
    ...
    warnings.warn("RichDEM not available. Advanced flow routing will use scipy methods.")
    rd = None
    HAS_RICHDEM = False
...
try:
    ...
    warnings.warn("Some preprocessing modules not available. Using simplified implementations.")
...
class RealDataPreprocessor:
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized RealDataPreprocessor with target CRS: {target_crs}, "
    ...
    def process_region(self, 
                      ...
                      output_dir: Optional[Union[str, Path]] = None) -> Dict[str, xr.DataArray]:
    ...
    def create_training_tiles(self,
        ...
        """
        logger.info(f"Creating training tiles: {tile_size}x{tile_size} with {overlap}px overlap")
    ...
    def _create_cache_key(self, dem_path: Union[str, Path], 
                         rainfall_data: Dict[str, Any],
                         region_bounds: Optional[Tuple]) -> str:
        """Create cache key for processing parameters."""
        key_data = {
            'dem_path': str(dem_path),
            'dem_mtime': Path(dem_path).stat().st_mtime if Path(dem_path).exists() else 0,
            'rainfall_data': str(sorted(rainfall_data.items())),
            'region_bounds': region_bounds,
            'target_crs': str(self.target_crs),
            'target_resolution': self.target_resolution,
            'config_hash': hashlib.md5(str(sorted(self.config.items())).encode()).hexdigest()
        }
...
class EnhancedDEMProcessor(BaseDEMProcessor if 'BaseDEMProcessor' in locals() else object):
    ...
    def process_usgs_3dep(self, 
                          dem_path: Union[str, Path],
                          bounds: Optional[Tuple[float, float, float, float]] = None) -> Dict[str, xr.DataArray]:
        ...
        if bounds:
            ...
            dem_da = dem_da.sel(
                x=slice(minx, maxx),
                y=slice(maxy, miny)  # y is typically decreasing
            )
    ...
    def _compute_advanced_flow(self, dem: xr.DataArray) -> Dict[str, xr.DataArray]:
        ...
        if self.has_richdem:
            ...
            results['flow_accumulation'] = derived.get('flow_accumulation', dem.copy(data=np.ones_like(dem.values)))
...
class EnhancedRainfallProcessor(RainfallGenerator if 'RainfallGenerator' in locals() else object):
    """Enhanced rainfall processor for NOAA Atlas 14 data."""
    
    def __init__(self, config: Optional[Dict] = None):
        if hasattr(super(), '__init__'):
            super().__init__(time_step_minutes=config.get('time_step_minutes', 15) if config else 15)
        self.config = config or {}
    
    def process_noaa_atlas14(self, 
                           rainfall_data: Dict[str, Any],
                           template_raster: xr.DataArray) -> Dict[str, xr.DataArray]:
        ...
        distributions = self.config.get('distribution_types', ['scs_type_ii', 'uniform'])
        ...
        for rp in return_periods:
            ...
            for duration in durations:
                if str(duration) not in rainfall_data[str(rp)]:
                    logger.warning(f"Duration {duration}h not found for {rp}-year return period")
                    continue
    ...
    def _create_rainfall_raster(self,
                               total_rainfall_mm: float,
                               template_raster: xr.DataArray,
                               distribution_type: str = 'uniform') -> xr.DataArray:
        """Create rainfall raster with optional spatial variability."""
        
        # Create base uniform field
        rainfall = np.full_like(template_raster.values, total_rainfall_mm, dtype=np.float32)
...
class SpatialProcessor:
    ...
    def _align_to_reference(self, raster: xr.DataArray, reference: xr.DataArray) -> xr.DataArray:
        """Align raster to reference grid."""
        # Check if already aligned
        if (raster.x.values.shape == reference.x.values.shape and 
            raster.y.values.shape == reference.y.values.shape and
            np.allclose(raster.x.values, reference.x.values, atol=self.target_resolution/10) and
            np.allclose(raster.y.values, reference.y.values, atol=self.target_resolution/10)):
            return raster
        
        # Interpolate to reference grid
        aligned = raster.interp(
            x=reference.x,
            y=reference.y,
            method='linear'
        )
...
class DataValidator:
    ...
    def validate_spatial_alignment(self, 
                                  ...
                                  terrain_data: Dict[str, xr.DataArray]) -> Dict[str, bool]:
        ...
        for name, array in rainfall_data.items():
            if isinstance(array, xr.DataArray):
                results[f'rainfall_{name}_aligned'] = self._check_alignment(reference, array)
        
        # Check terrain features alignment
        for name, array in terrain_data.items():
            if isinstance(array, xr.DataArray):
                results[f'terrain_{name}_aligned'] = self._check_alignment(reference, array)
    ...
    def _check_alignment(self, reference: xr.DataArray, test_array: xr.DataArray) -> bool:
        ...
        try:
            ...
            x_aligned = np.allclose(reference.x.values, test_array.x.values, 
                                  atol=self.tolerance)
            y_aligned = np.allclose(reference.y.values, test_array.y.values, 
                                  atol=self.tolerance)
...
class EnhancedTileGenerator:
    ...
    def __init__(self, tile_size: int = 512, overlap: int = 64, config: Optional[Dict] = None):
    ...
    def generate_tiles(self, 
                      ...
                      flood_labels: Optional[xr.DataArray] = None) -> List[Dict]:
        """Generate training tiles from multiple data arrays."""
        logger.info(f"Generating tiles: {self.tile_size}x{self.tile_size} with {self.overlap}px overlap")
