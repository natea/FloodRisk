...
class PreprocessingQA:
    ...
    def run_comprehensive_qa(self, 
        ...
        qa_results['spatial_validation'] = self._validate_spatial_consistency(processed_data)
        ...
        qa_results['consistency_validation'] = self._validate_consistency(processed_data)
        
        # Performance metrics
        logger.info("Computing performance metrics")
        qa_results['performance_metrics'] = self._compute_performance_metrics(processed_data)
    ...
    def _validate_data_quality(self, data: Dict[str, xr.DataArray]) -> Dict[str, Any]:
        ...
        for name, array in data.items():
            ...
            array_validation['nodata_acceptable'] = nodata_percent <= self.config['thresholds']['nodata_percent_max']
    ...
    def _validate_spatial_consistency(self, data: Dict[str, xr.DataArray]) -> Dict[str, Any]:
        ...
        validation['reference_crs'] = str(reference.rio.crs) if hasattr(reference, 'rio') else 'unknown'
        ...
        for i, array in enumerate(arrays[1:], 1):
            ...
            if hasattr(array, 'x') and hasattr(reference, 'x'):
                x_aligned = np.allclose(array.x.values, reference.x.values, 
                                      atol=self.config['thresholds']['alignment_tolerance_m'])
                y_aligned = np.allclose(array.y.values, reference.y.values,
                                      atol=self.config['thresholds']['alignment_tolerance_m'])
            ...
            if hasattr(array, 'rio') and hasattr(reference, 'rio'):
                try:
                    ...
                    res_match = (abs(ref_res[0] - arr_res[0]) < self.config['thresholds']['spatial_resolution_tolerance'] and
                               abs(ref_res[1] - arr_res[1]) < self.config['thresholds']['spatial_resolution_tolerance'])
        ...
        validation['spatial_consistency'] = validation['all_shapes_aligned'] and validation['all_resolutions_match']
    ...
    def _validate_consistency(self, data: Dict[str, xr.DataArray]) -> Dict[str, Any]:
        ...
        if 'flow_accumulation' in data and 'streams' in data:
            validation['flow_streams_consistent'] = self._check_flow_streams_consistency(
    ...
    def _compute_performance_metrics(self, data: Dict[str, xr.DataArray]) -> Dict[str, Any]:
        ...
        metrics['avg_memory_per_pixel'] = total_memory / total_pixels if total_pixels > 0 else 0
    ...
    def _check_reasonable_range(self, name: str, data: np.ndarray) -> bool:
        ...
        return not (np.any(np.isinf(data)) or np.any(data < -1e10) or np.any(data > 1e10))
    ...
    def _check_dem_slope_consistency(self, dem: xr.DataArray, slope: xr.DataArray) -> bool:
    ...
    def _check_flow_streams_consistency(self, flow_acc: xr.DataArray, streams: xr.DataArray) -> bool:
    ...
    def _check_hand_dem_consistency(self, hand: xr.DataArray, dem: xr.DataArray) -> bool:
        ...
        return np.all(valid_hand >= 0) and np.all(valid_hand <= 1000)  # Reasonable upper bound
    
    def _compute_feature_correlations(self, data: Dict[str, xr.DataArray]) -> Dict[str, float]:
    ...
    def _generate_qa_visualizations(self, 
        ...
        if 'correlations' in results['consistency_validation']:
            self._plot_correlation_heatmap(results['consistency_validation']['correlations'], viz_dir)
    ...
    def _plot_data_overview(self, data: Dict[str, xr.DataArray], output_dir: Path):
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "data_overview.png", dpi=self.config['visualization']['dpi'], bbox_inches='tight')
        plt.close()
    ...
    def _plot_statistical_summaries(self, stats: Dict[str, Dict], output_dir: Path):
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "statistical_summary.png", dpi=self.config['visualization']['dpi'], bbox_inches='tight')
        plt.close()
    
    def _plot_correlation_heatmap(self, correlations: Dict[str, float], output_dir: Path):
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "correlation_heatmap.png", dpi=self.config['visualization']['dpi'], bbox_inches='tight')
        plt.close()
    ...
    def _plot_quality_dashboard(self, results: Dict[str, Any], output_dir: Path):
        ...
        status_text += f"Total Memory: {perf_metrics.get('total_memory_mb', 0):.1f} MB\n"
        ...
        axes[1, 1].text(0.1, 0.5, status_text, transform=axes[1, 1].transAxes,
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "quality_dashboard.png", dpi=self.config['visualization']['dpi'], bbox_inches='tight')
        plt.close()
        ...
        with open(summary_file, 'w') as f:
            ...
            for name, metrics in results['data_validation'].items():
                if isinstance(metrics, dict):
                    ...
                    f.write(f"    Range Valid: {metrics.get('range_valid', 'Unknown')}\n")
                    f.write(f"    Min/Max: {metrics.get('min_value', 'N/A')}/{metrics.get('max_value', 'N/A')}\n\n")
            
            # Spatial validation summary
            f.write("Spatial Validation:\n")
            f.write("-" * 18 + "\n")
            spatial = results['spatial_validation']
            f.write(f"  Shapes Aligned: {spatial.get('all_shapes_aligned', 'Unknown')}\n")
            f.write(f"  Resolutions Match: {spatial.get('all_resolutions_match', 'Unknown')}\n")
            f.write(f"  Overall Consistent: {spatial.get('spatial_consistency', 'Unknown')}\n\n")
    ...
    def _store_metrics(self, results: Dict[str, Any]):
        ...
        metric = QAMetrics(
            ...
            value=1.0 if overall_status == 'pass' else 0.5 if overall_status == 'warning' else 0.0,
...
def run_qa_pipeline(processed_data: Dict[str, xr.DataArray], 
