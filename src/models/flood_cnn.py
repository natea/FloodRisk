...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        spatial_att = self.sigmoid(
            nn.Conv2d(2, 1, kernel_size=7, padding=3, bias=False).to(x.device)(
                spatial_concat
            )
        )
...
class PhysicsConstraintModule(nn.Module):
    ...
    def compute_gradients(
        self, flood_depth: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Compute spatial gradients using Sobel operators."""
        grad_x = F.conv2d(flood_depth, self.sobel_x, padding=1) / (
            8 * self.grid_spacing
        )
        grad_y = F.conv2d(flood_depth, self.sobel_y, padding=1) / (
            8 * self.grid_spacing
        )
...
class MultiScaleUNet(nn.Module):
        ...
        self.fusion_modules = nn.ModuleDict(
            {
                ...
                "level_2": FeatureFusion(
                    [base_channels * 2] * len(scales), base_channels * 2
                ),
                "level_3": FeatureFusion(
                    [base_channels * 4] * len(scales), base_channels * 4
                ),
                "level_4": FeatureFusion(
                    [base_channels * 8] * len(scales), base_channels * 8
                ),
            }
        )
    ...
    def forward(
        self, x: torch.Tensor, rainfall: Optional[torch.Tensor] = None
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        for scale in self.scales:
            ...
            for i, (conv_block, pool) in enumerate(
                zip(encoder_layers[:-1], pool_layers)
            ):
        ...
        for i, level in enumerate(["level_4", "level_3", "level_2", "level_1"]):
            fused = self.fusion_modules[level](scale_features[level], target_sizes[i])
...
if __name__ == "__main__":
    ...
    print(
        f"Output range: [{flood_depth.min().item():.3f}, {flood_depth.max().item():.3f}]"
    )
